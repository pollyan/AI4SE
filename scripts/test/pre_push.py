...
def fixed_suites(context: PrePushContext) -> tuple[SuiteSpec, ...]:
    ...
    return (
        ...
        _suite(
            ...
            cwd=context.root,
            invariants=("real-model evidence, stream and persistence assertions",),
            evidence_level=2,
            external_dependency="none",
            disposition="KEEP",
        ),
        _suite(
            phase="deterministic",
            suite_id="new-agents-live-stack",
            parser="pytest",
            command=(
                python,
                "-m",
            ...
            cwd=context.root,
            invariants=("real frontend/backend/SSE/browser deterministic boundary",),
            evidence_level=3,
            external_dependency="deterministic provider seam",
            disposition="MERGE",
        ),
        _suite(
            phase="deterministic",
            suite_id="new-agents-browser-e2e",
            parser="pytest",
            command=(
                python,
                "-m",
            ...
            cwd=context.root,
            invariants=("browser-visible stream order and artifact rendering",),
            evidence_level=3,
            external_dependency="controlled Agent Runtime seam",
            disposition="KEEP",
        ),
    )
