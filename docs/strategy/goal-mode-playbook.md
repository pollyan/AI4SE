...
- **必需产物**：只覆盖当前厚切片的中文 implementation plan，明确 TDD 或文档验证路径、预计修改范围、文件所有权、子智能体边界、聚焦与跨层验证、全量与 CI 等价边界、聚焦提交和交付边界；所有清单和任务必须标识为内部实现步骤（非切片）。
...
- **进入条件**：`IMPLEMENT` 的聚焦验证已达到进入门槛且最终实现和必要文档已经落定；或 `DELIVER` 发现原实现验证证据失效，需要按当前事实重新验证。
