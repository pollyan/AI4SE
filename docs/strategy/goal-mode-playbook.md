...
13. 目标模式下，完成一个通过验证的完整用户故事、独立工程信任闭环或其他可独立说明价值的批次后，必须先批量运行该批次对应的聚焦验证、全量自动化或明确记录的验证例外，再形成聚焦 commit；如果 GitHub 远端可用、当前分支允许推送且用户没有明确要求暂缓，commit 后应及时 push 到 GitHub。进入下一用户故事前不得把多个独立价值点或大体量 diff 长期缓存在本地未提交状态。工作区存在无关大 diff 时，不能因此跳过提交；只要本轮改动能被明确列出并安全 staged，就必须只 stage 本轮路径、提交并推送。
...
普通任务默认不要自动 commit，除非用户本轮明确要求或任务本身规定必须提交。目标模式不同：每个完成必要实现、验证和文档记录的完整用户故事、独立工程信任闭环或可独立说明价值的批次，都应在进入下一用户故事前先完成批量验证、创建聚焦 commit，并在 GitHub 远端可用且没有明确暂缓条件时及时 push。这个提交节奏是目标模式的默认交付边界，不应把多个用户可感知能力包或上万行 diff 长期堆积在本地。
...
6. 每个稳定用户故事 commit 后默认应及时 push 到 GitHub；如果需要把几个紧密相关的 checkpoint commits 作为一个批次统一推送，必须先完成该批次的验证，并在收尾说明中列出该批次包含的独立价值点。
...
- 除非我明确要求不要 commit / push，否则完成型用户故事、独立工程信任闭环或可独立说明价值的批次应形成聚焦 commit，并在 GitHub 远端可用时及时 push。
