...
- **离开门禁**：非终态 todo 已更新当前切片状态与下一入口；终态 todo 已完成第 6.1 节归档事务；本轮新增 staged 集只包含本轮文件且用户原有 staged 项保持原样；已形成一个对应完整能力包、工程信任闭环、bugfix 或稳定记录收口的聚焦 commit，或符合第 6.2 节暂缓条件、记录完整且 index 状态符合用户明确安排；交付说明与当前 `HEAD`、远端和未提交 diff 一致。
