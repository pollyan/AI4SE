...
无论用户是否指定本轮目标，`BOOTSTRAP` 都必须扫描活跃 todo 的 P0/P1、非归档 todo 的终态 / 归档一致性和当前失败证据。未指定目标时，从活跃 todo、待收口 todo、失败证据、专项计划和相关 backlog 聚合候选并执行完整 CGA；已指定目标时，同向、低冲突且可形成厚切片的高优先级项必须进入评估，未纳入本轮时记录原因和去向。
...
| `docs/todos/**` 中的非归档 todo | 活跃能力包，或已终结但尚待归档的状态和结果证据 | CGA、TDD、子智能体、提交规则、逐步 implementation plan、长命令输出 |
| `docs/todos/archive/**` | 已终结 todo 的历史证据 | 默认候选、当前入口、待执行指令 |
...
长期 todo 只需稳定记录能力包承接的需求、目标态、边界、依赖和结果证据；执行时由本文重新检查当前事实并生成当轮 spec、plan 与验证记录。单个切片终结时更新活跃 todo；整份 todo 终结时按第 6.1 节归档，不让历史记录继续充当当前入口。
...
stateDiagram-v2
    ...
    ASSESS --> DELIVER: 无需实现的稳定记录收口
    ASSESS --> NEXT: 无需归档且没有待交付记录
    ASSESS --> WAIT: 暂停条件成立
    ...
    DELIVER --> VERIFY: 实现验证证据失效
    DELIVER --> ASSESS: 终态或决策证据失效
    DELIVER --> WAIT: 暂停条件成立
...
- **必需产物**：当前用户目标与最新反馈；当前分支和 `git status`；用户已有改动、可写与禁止触碰边界；实际读取的稳定事实源、活跃或待收口 todo、相关代码与测试；未关闭质量门、阻断测试、外部阻塞和遗留风险；子智能体能力与可并行事项判断。
...
- **离开门禁**：一个且仅一个当前能力包被推荐或继续承接，优先级、边界、依赖和可观察证据都有当前事实支持；或当前能力包被明确取消、替换、移出序列或已由当前事实证明满足，并已判断 owning todo 是否存在、是否仍活跃、是否需要归档以及是否产生必须落盘的记录。需要稳定记录收口时进入 `DELIVER`；无需归档（包括 owning todo 不存在或仍活跃）且没有待交付记录时才可直接进入 `NEXT`。
- **失败路由**：需要重排、拆并或修复风险时留在 `ASSESS`；外部条件缺失进入 `WAIT`；不得用旧计划强行继续。

### 2.3 `MILESTONE`

- **进入条件**：`ASSESS` 已选定或承接一个候选能力包。
- **必需产物**：从用户完整任务命名的厚切片，明确角色、真实场景和独立价值；纳入与排除边界、依赖、能力增量句、验收条件，以及第 3 节七项门禁结论；按第 3.4 节确定临时切片身份基线，用户已确认有序能力包时同时定位 owning todo 中的顺序基线。
...
- **离开门禁**：需求和架构边界清楚，所选方案及不选理由可审计，成功与失败证据可以被计划化；临时切片身份基线已经持久化到中文 spec 的“厚切片身份基线”章节，且用户已确认有序能力包时已链接 owning todo 的顺序基线。
- **失败路由**：需求、架构、事实或目标态无法自洽时回到 `ASSESS`；不得靠实现假设填补设计冲突。

### 2.5 `PLAN`

- **进入条件**：中文 spec 已完成且无未决设计冲突。
- **必需产物**：只覆盖当前厚切片的中文 implementation plan，明确 TDD 或文档验证路径、预计修改范围、文件所有权、子智能体边界、聚焦与跨层验证、全量与 CI 等价边界、第 4.5 节厚切片级正式审查及高风险契约冻结检查点决定、聚焦提交和交付边界；所有清单和任务必须标识为内部实现步骤（非切片）。
- **离开门禁**：计划可按小步执行并最终形成一个独立价值点，所有写入路径和验证责任有唯一归属，且第 3.4 节厚切片身份稳定性检查通过。
- **失败路由**：只有多个隐藏内部批次完成后才首次产生价值，或无法形成单一交付边界时回到 `ASSESS` 重新划分。

### 2.6 `IMPLEMENT`

- **进入条件**：implementation plan 通过门禁且工作区写入边界安全。
- **必需产物**：行为变更的 RED/GREEN 证据或纯文档变更的一致性证据；计划内实现；主 Agent 对所有子智能体输出的 diff 与证据复核；第 4.5 节厚切片级正式审查及必要复审证据；必要的局部记录更新。
- **离开门禁**：计划内实现完成，最快的确定性聚焦验证满足预期，正式审查的 Critical / Important 发现已经关闭并有复审证据，没有越界写入、silent fallback、伪造成功或未处理的实现缺口。
- **失败路由**：聚焦验证失败时留在 `IMPLEMENT`；新 P0/P1、目标失效或边界错误回到 `ASSESS`；外部实现条件缺失进入 `WAIT`。

### 2.7 `VERIFY`

- **进入条件**：`IMPLEMENT` 的聚焦验证和第 4.5 节正式审查已经达到进入门槛，最终实现和必要文档已经落定；或 `DELIVER` 发现原实现验证证据失效，需要按当前事实重新验证。
...
- **进入条件**：`VERIFY` 的不可豁免关键门禁全部通过且其他缺口满足第 5.1 节受控例外；或 `ASSESS` 已证明当前能力包无需行为实现，只需落盘取消、替换、迁移、非终态切片更新或终态 todo 归档。
- **必需产物**：稳定事实源和 todo 的必要更新及第 6.1 节要求的终态判断，todo 终态时还必须完成归档事务；启动前用户已有 staged 项与本轮新增 staged 项的精确清单；聚焦 commit 或合法暂缓记录；push 决定；完整交付说明。
- **离开门禁**：非终态 todo 已更新当前切片状态与下一入口；终态 todo 已完成第 6.1 节归档事务；本轮新增 staged 集只包含本轮文件且用户原有 staged 项保持原样；已形成一个对应完整能力包、工程信任闭环、bugfix 或稳定记录收口的聚焦 commit，或符合第 6.2 节暂缓条件、记录完整且 index 状态符合用户明确安排；交付说明与当前 `HEAD`、远端和未提交 diff 一致；第 3.4 节厚切片身份稳定性检查通过，内部步骤未被记为独立完成、提交或交付。
- **失败路由**：实现验证证据失效回到 `VERIFY`；终态判断、取消 / 替换 / 迁移依据、owning 去向或陈旧完成证据失效回到 `ASSESS`；路径、引用、staging、记录或提交归属问题留在 `DELIVER` 修正；外部条件缺失进入 `WAIT`。不得把未完成归档带入下一轮。

### 2.9 `NEXT`

- **进入条件**：当前厚切片真正完成，或经 `ASSESS` 明确取消、替换或移出当前序列；若 owning todo 已终结，第 6.1 节归档事务必须已经完成。
- **必需产物**：已完成与剩余能力包清单、当前残余风险、下一候选来源；若报告百分比，同时给出能力包依据。
- **离开门禁**：当前轮没有被外部阻塞伪装成完成，已终结 todo 不再位于活跃入口，下一轮必须重新读取当前事实。
- **失败路由**：发现当前厚切片未完成或终态 todo 尚未归档时回到 `BOOTSTRAP` 重建事实，再由 `ASSESS` 定位应恢复的状态并重新通过沿途门禁；外部阻塞进入 `WAIT`。不得从 `NEXT` 直接跳回实现或验证，也不得自动跳过当前能力包。
...
`MILESTONE` 通过时必须确定当前厚切片的临时身份基线：ID、名称、完整用户任务、纳入与排除边界、七项门禁、依赖、验收证据和单一交付边界。完成 brainstorming 后，`DESIGN` 必须把临时基线持久化到当前中文 spec 的“厚切片身份基线”章节，该章节是身份基线的唯一持久位置；用户已经确认有序能力包时，其 owning todo 的能力包序列是顺序基线的唯一持久位置。implementation plan、进度记录和交付说明只能链接这两个位置及标题，不得复制后另行改写。只要当前事实仍支持这些基线，后续状态必须按原切片继续执行，不得由 Agent 私自改名、缩小、拆薄、跳过或重排；跨窗口、上下文压缩、更换 Agent 或恢复执行时也必须重新读取这些位置。

implementation plan 可以列出多个 RED/GREEN 步骤、测试、helper、子智能体任务或检查点，但它们必须明确标识为“内部实现步骤（非切片）”。`A/B/C`、小数编号、`phase`、`batch`、`wave`、`checkpoint`、`task` 或任何其他内部标签都不得获得切片语义：不得据此单独编写 spec、计算厚切片进度、宣称验收或完成、形成独立 commit / push / 交付，也不得把尚未闭合七项门禁的半条链路包装成阶段成果。内部步骤完成只能更新当前厚切片的实施证据，不能增加已完成厚切片数。

真正需要改界时，必须先满足第 3.1 节强制改道条件并回到 `ASSESS`。只有不同用户目标、入口、角色、数据契约、下游依赖，或能独立闭合七项门禁的显著不同风险面已经形成自然业务边界时，才允许用完整 CGA 取消、替换、合并或拆分原切片；工作量大、文件多、上下文不足、测试耗时、需要换窗口或 Agent、想先保存 checkpoint 或便于提交都不是自然业务边界或改界依据。重新划分后的每一项必须是拥有独立用户任务和完整七项门禁的同级厚切片，并记录原 ID、successor、顺序变化、剩余工作 owner 和证据。禁止用 `QS-03A/QS-03B`、`4A/4B` 等父编号变体延续私有碎片。

从 `MILESTONE` 起，每次离开 `DESIGN`、`PLAN`、`IMPLEMENT`、`VERIFY`、`DELIVER` 或 `NEXT` 前，都必须检查并在当轮产物中可定位以下事实：

1. 当前切片身份和用户已确认顺序是否仍与基线一致；如不一致，是否已经按强制改道路由回到 `ASSESS`。
2. plan、进度、验收、commit、push 和交付是否仍只以完整厚切片记账，内部标签是否始终保持“非切片”语义。
3. 当前剩余工作是否仍映射到原七项门禁和单一交付边界；任一门禁未闭合时是否仍停留在当前厚切片。
4. 是否出现伪装子切片编号、内部步骤完成百分比替代厚切片计数、内部 checkpoint 独立交付，或用局部验证宣称整片完成；任一项出现都不得转换状态，必须先修正规划、记录和提交边界。

当前厚切片身份已经冻结时，执行中发现并修复的 bug 默认属于该厚切片，不能仅因可以命名为 bugfix 就独立提交。只有完整 CGA 证明该 bug 中断或替代当前切片、拥有独立用户任务和完整七项门禁，并为它建立新的同级身份基线后，才允许作为独立 bugfix 交付；原切片必须同时记录取消、替代、恢复或顺序去向。
...
厚切片纵向不等于可以把内部实现按技术层横向排队。implementation plan 的内部 `Task` / 步骤也必须优先围绕可运行 tracer 场景组织，把该场景所需的配置、契约、后端、前端、部署与验证放在同一 RED→GREEN 链内；禁止形成“先全部配置、再全部后端、再全部前端、最后才部署/E2E”的流水线，或把首次端到端证据推迟到最后一个内部步骤。确有多个场景共享 seam 时，可以先建立最小 seam，但紧邻步骤必须立即由一个真实场景消费并形成跨边界证据；共享 seam 本身仍不是切片、进度、commit 或交付点。若多个技术风险面能够各自闭合入口、动作、状态承接、失败反馈和证据，只因共享主题而被合并，必须在 `PLAN` 前回到 `ASSESS` 按自然边界重新划分，不能用“大而完整”替代纵向可执行性。

上述纵向原则同样适用于技术重构、安全整改、基础设施和部署变更，不只适用于新增业务功能。此时 tracer 的单位可以是一个可观察的系统行为或风险闭环，而不必伪装成新 UI 功能；例如“入口请求经过新 seam 后保持原行为”“跨语言凭据从签发到消费可用”“缺配置在启动副作用前失败”“网关到服务的静态资源与健康路径可达”。每个重构步骤仍必须形成 RED → 最小实现 → 聚焦回归 → 跨边界证据，只有代码层、模块或文件集完成而没有真实消费方证据时不得记为完成。为了小步快跑，应优先选择能独立闭合且能及时回归的最小自然行为边界；不能用固定行数硬拆，也不能把多个可独立验证的风险面捆成一次大爆炸重构。
...
正式代码审查默认以完整厚切片为唯一单位，在计划内所有内部实现步骤完成、聚焦验证通过且候选最终 diff 已落定后执行一次，覆盖 **Spec**（是否满足当前中文 spec、厚切片身份基线和验收条件）与 **Standards**（架构、正确性、安全性、可维护性、测试和仓库规范）两个维度。两个维度可以并行读取同一份审查包，但属于同一轮厚切片审查；不得把它们记为两个切片、两个交付点或两个完成进度。

“审查包”是正式代码审查的输入集合，不是额外实现阶段或独立产物门禁。它至少包含当前 spec 与 implementation plan 的链接、厚切片完整变更范围与 ownership、最终 diff 或可精确重建该 diff 的基线、关键风险与契约变化、聚焦及必要跨层验证证据、已知缺口和生成产物处理方式。压缩、minified、vendored、二进制或机械生成内容可以不进入逐行文本审查，但必须通过来源、hash、构建、生成一致性或消费方验证证明其身份与影响；不得借此排除手写业务逻辑或掩盖未审查变更。

内部 `Task`、RED/GREEN 小步、helper、测试批次和子智能体写入不各自生成正式审查包、不各自派发完整代码审查，也不设置独立审查门禁。它们仍必须完成 TDD / 文档一致性检查、最小聚焦验证、ownership 检查和主 Agent 的即时 diff 复核；发现问题立即修正，不能积压已知失败到厚切片末尾。执行记录在 RED、核心实现完成和 GREEN 三个有事实变化的节点增量更新，供最终审查包复用，不重复抄写累计上下文。

每个厚切片最多允许一个正式审查前的“高风险契约冻结检查点”。只有安全边界、共享运行时、公共 API / schema、不可逆数据迁移或其他会被大量后续实现依赖、晚发现将造成显著返工的契约，才可在 `PLAN` 中预先指定检查点、范围和触发证据。检查点只审查该风险边界，不替代最终厚切片审查，不形成独立 commit、验收、交付或切片进度；普通文件多、diff 大、内部 Task 完成或希望阶段性确认都不是设置检查点的理由。实现中才出现新的高风险边界时，按第 3.1 节判断是否需要回到 `ASSESS`，不得临时增设多轮检查点。

正式审查发现阻断问题时留在当前厚切片修复。首次审查读取完整厚切片；修复后的复审默认只提供原发现、修复 delta、相邻影响面和新增验证证据，只有修复改变了既定契约或影响范围时才重新执行完整审查。所有 Critical / Important 发现必须关闭并留下复审证据后才能通过 `IMPLEMENT` 离开门禁；未处理的 Minor 必须有明确风险、owner 和去向，不能用“已审查”替代修复或第 5 节验证。
...
`docs/todos/` 下除 `archive/**` 外的 todo 文档在完成归档前都必须由 `BOOTSTRAP` 审计：仍承载可执行能力包或被当前入口列为待执行的是活跃 todo；已经终结但尚未归档的是待收口 todo，不再作为实现候选，但必须经 `ASSESS → DELIVER` 完成归档。README、索引和归档记录本身不是 todo 候选。切片只有在完成，或经 `ASSESS` 明确取消、替换、迁移后才进入终态：取消必须记录依据与影响，替换或迁移必须记录新的 owning 路径；影响实现、验收或不可豁免门禁的外部阻塞不是终态，第 5.1 节受控例外仍须保留原始非 `PASS` 状态、风险与恢复条件。

单个切片终结但 todo 仍有其他能力包时，只更新原 todo 的状态、证据和下一入口。只有所有切片均进入终态、没有未解释的 checkbox 或残余 P0/P1、需要保留的 P2/P3 已迁入明确 owning backlog，且完成事实已进入当前分支的已验证待交付工作树时，整份 todo 才进入终态；其他分支、worktree 或未合流远端中的结果不能单独作为归档依据。

终态 todo 必须在 `DELIVER` 中作为一个事务完成：更新总状态与切片证据；把整份文件移入 `docs/todos/archive/`；清理 README、索引和其他当前 / 未来执行入口；逐条保留明确属于历史事实的旧路径引用。目标归档路径已存在时禁止覆盖，必须比较双方独有证据并显式确定 canonical 文件或消歧名称。随后验证归档文件存在、旧活跃路径消失、活跃索引不再列出它，并完成文档一致性、`git diff --check` 与 staged ownership 检查。

本轮最后切片与归档事务默认进入同一聚焦 commit；实现已在先前 `HEAD` 时可形成独立归档收口 commit。离开 `DELIVER` 时，当前交付事实必须同时包含完成事实和完整归档事务：正常提交时由新 `HEAD` 承载，满足第 6.2 节合法暂缓条件时则精确记录未提交归档状态和下一触发条件。归档事务本身任一步未完成都留在 `DELIVER`，不得进入 `NEXT`。
...
本轮目标：<目标；未指定时从活跃 / 待收口 todo、失败证据和当前代码事实中选择>。
