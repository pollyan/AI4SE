...
8. 第一个实质性工作产物必须是完整 Current State Gap Analysis；若本轮是已确认目标轮次的连续执行，则第一个实质性工作产物必须是目标承接检查。该产物必须包含子智能体 / 旁路审查决策：可并行读取、审查、验证或互不重叠实现时优先分发；不分发时写清原因。
...
13. 目标模式下，完成一个通过验证的完整用户故事、独立工程信任闭环或其他可独立说明价值的批次后，必须先批量运行该批次对应的聚焦验证、全量自动化或明确记录的验证例外，再形成聚焦 commit；如果 GitHub 远端可用、当前分支允许推送且用户没有明确要求暂缓，commit 后应及时 push 到 GitHub。进入下一用户故事前不得把多个独立价值点或大体量 diff 长期缓存在本地未提交状态。
14. 只有用户明确要求不要提交或不要 push、当前只是临时探索、验证未完成、存在未裁决风险、工作区存在无法安全拆分的无关脏改动、缺少远端权限 / 凭证或外部服务不可用时，才可以暂缓 commit / push，并必须在收尾说明中写明原因、风险和下一次提交 / push 触发条件。
15. 收尾说明必须列出改动、验证命令、未运行验证及原因、CI 等价覆盖判断、子智能体 / 旁路审查的派发或跳过原因、LLM judge 分数与低于 80 分时的修复 / 暂缓记录、残余风险、提交 / push 状态、`HEAD` 与 GitHub 远端同步状态和下一步候选。
...
- 子智能体调度也是验证记录的一部分。每轮必须说明是否存在可并行只读审查、验证或互不重叠实现切片；存在且环境支持时优先分发，不存在或不适合时记录跳过原因。LLM judge 低分、跨 workflow 覆盖矩阵、契约同步和浏览器级证据审查属于默认优先考虑子智能体的场景。
