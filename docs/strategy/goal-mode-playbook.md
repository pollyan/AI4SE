...
如果用户没有指定本轮目标，默认从 `docs/plans/tech-debt.md` 和 `docs/todos/` 中选择最高优先级、尚未完成、当前上下文可推进的 New Agents 工作项；但每轮仍必须先做 Current State Gap Analysis，不能直接套用上一轮结论或固定清单。
...
13. `docs/todos/`
14. 与本轮相关的 `docs/superpowers/specs/`、`docs/superpowers/plans/`、测试需求、当前代码、测试、脚本和 git 历史。
...
9. 更新必要的文档记录，例如 `docs/plans/tech-debt.md`、`docs/todos/`、相关 spec / plan 或测试策略说明。
...
5. `docs/plans/tech-debt.md` 中高优先级且可形成纵向切片的工作项。
...
本轮目标：
- <写明长期目标或当前阶段目标>
- 如果没有更高优先级用户指令，优先从 `docs/plans/tech-debt.md` 和 `docs/todos/` 中选择最高优先级、尚未完成、当前可推进的工作项。
