...
13. 目标模式下，完成一个通过验证的完整用户故事、独立工程信任闭环或其他可独立说明价值的批次后，默认必须形成聚焦 commit；如果 GitHub 远端可用、当前分支允许推送且用户没有明确要求暂缓，commit 后应及时 push 到 GitHub，避免多个独立价值点长期堆积在本地未提交 diff 中。
...
普通任务默认不要自动 commit，除非用户本轮明确要求或任务本身规定必须提交。目标模式不同：每个完成必要实现、验证和文档记录的完整用户故事、独立工程信任闭环或可独立说明价值的批次，都应在进入下一用户故事前先完成批量验证、创建聚焦 commit，并在 GitHub 远端可用且没有明确暂缓条件时及时 push，避免多个用户可感知能力包或上万行 diff 长期堆积在本地。
