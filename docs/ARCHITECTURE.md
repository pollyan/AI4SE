...
| `/intent-tester/static/` | `intent-tester:5001` | Intent 自有 Jinja/JS/CSS/vendor 资产；不得落到 portal `/static/` |
...
├── intent_security/    → 单一 config/policy/session/Origin-CSRF/CSP/ticket 安全 seam
...
`install_intent_security(app)` 是唯一安全安装入口，在 SQLAlchemy 初始化前完成 production preflight 和 URL-map policy completeness。`restricted` 与 `public-readonly` 共用同一路由、session、Origin/CSRF、CSP 和 renderer；`local-dev` 只允许非 production loopback。生产只有 Nginx 发布 host port，Flask、PostgreSQL 和 managed Node 保持内部；Node 使用 service DNS、backend bearer 和同一 canonical execution ID。native local-host package 只绑定 loopback，浏览器先向 Flask 取得短期同 ID ticket，再连接 Node 的 execution room。Flask durable record 与 terminal lifecycle 始终是唯一状态权威。
...
| **Alex** | 业务分析师 | 创意头脑风暴 (4 阶段)、需求蓝图梳理 (4 阶段)、用户故事拆解 (4 阶段)、PRD 质量评审与补全 (4 阶段) |
...
|------|------|------|
| GET | `/api/health` | 健康检查 |
| GET | `/api/config` | 获取 LLM 配置与浏览器配置管理 capability（不含 API Key） |
| POST | `/api/config` | 创建或更新默认 LLM 配置（不返回 API Key） |
| POST | `/api/config/check` | 检测默认 LLM 配置或设置表单临时配置是否可调用当前模型 |
| POST | `/api/config/default/check` | 只检测服务端默认模型且不接受目标覆盖 |
...
- `POST /api/config` 与 `POST /api/config/check` 只接受独立的 `NEW_AGENTS_CONFIG_ADMIN_API_KEY`；运行时代理密钥和固定网关标记都不能授权配置管理，生产缺密钥或模型/管理/代理三类密钥任意碰撞时 fail-closed
...
- `POST /api/config/check` 是受控管理员检测入口；`POST /api/config/default/check` 是浏览器运行时诊断入口，只能检测服务端目标且不接受请求体覆盖；两者均返回稳定公开消息，不回显供应商异常或密钥
...
- 前端同时读取 `hasDefault` 和服务端权威的 `browserConfigAdminAvailable`。只有明确启用匿名管理的 loopback 开发环境显示配置写入；生产设置弹层只读，更新由受控管理员 API 或部署 Secret 完成
