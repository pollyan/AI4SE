...
本索引是项目知识库的主入口。[AGENTS.md](../AGENTS.md) 是 AI Agent 的行为准则与工作指引；本文件负责按场景指向架构、API、测试、组件和部署等深层文档。
...
本文档是项目知识库入口。代码、路由、测试命令或模块职责变化时，需要同步更新本文件和对应深层文档：

1. 全局结构、服务拓扑或运行链路变化时，同步 `docs/ARCHITECTURE.md`。
2. API、SSE 事件或请求响应变化时，同步 `docs/api-contracts.md`。
3. 测试门禁、命令或覆盖策略变化时，同步 `docs/TESTING.md`。
4. 模块清单、推荐阅读路径或 AI 辅助开发上下文变化时，同步本文件。
