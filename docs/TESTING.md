...
- raw JSON streaming 的 partial `artifact_data` 不能被合成为 `artifact_update.replace` 进度页；只有完整解析并通过契约校验后的正式 Markdown 才能进入右侧 artifact。
...
可选 LLM judge 要求模型只返回严格 JSON verdict，至少包含 `pass`、`score`、`dimension_scores`、`issues`、`evidence` 和 `recommendations`。Lisa judge 侧重测试专家维度，Alex judge 侧重业务分析师维度，两者都包含交互体验和可视化维度。
