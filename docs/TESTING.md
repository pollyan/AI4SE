...
| `USER_STORY_BREAKDOWN/SCOPE` | 文档信息、范围内需求、追溯索引、不拆范围、阻塞问题、阶段门禁 | requirement trace 只校验不派生 | 后端由 `traceability_index` 生成 Mermaid `flowchart` | `test_render_user_story_breakdown_artifact_data_is_deterministic_and_contract_valid`、`REQUIRED_ARTIFACT_MERMAID_DIAGRAMS` |
| `USER_STORY_BREAKDOWN/STORY_MAP` | 文档信息、需求、活动、任务、故事地图项、MVP slice、Release slice、阶段门禁 | story/activity/task 关系只校验不派生 | 后端由 `story_map_items` / `tasks` 生成 Mermaid `flowchart` | `test_runtime_raw_json_stream_turn_renders_all_user_story_breakdown_stages_from_artifact_data`、visual contract maps |
| `USER_STORY_BREAKDOWN/STORIES` | 文档信息、需求、拆分原则、故事卡、Ready 汇总、Not Ready 故事、开放问题、阶段门禁 | storyId / requirementId 引用和 Ready / Not Ready 状态只校验不派生 | 无 required visual；后端渲染用户故事卡片和 Ready / Not Ready 表格 | `test_render_partial_user_story_breakdown_stories_artifact_data_builds_formal_incremental_markdown_and_patch`、story handoff tests |
| `USER_STORY_BREAKDOWN/HANDOFF` | 文档信息、需求、Ready 总览、单故事需求包、上游追溯、Not Ready 阻塞项、AI Coding 输入边界、阶段门禁 | 单故事包 trace / boundary 只校验不派生 | 无 required visual；后端渲染单故事需求包 JSON 和 AI Coding 输入边界 | `test_render_user_story_breakdown_artifact_data_is_deterministic_and_contract_valid`、single story handoff tests |
