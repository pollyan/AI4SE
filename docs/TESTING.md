...
| `REQ_REVIEW/REVIEW` | 评审信息、评审范围、质量总览、问题统计描述、问题分组、修订建议、阶段门禁 | `issue_statistics.p0_count/p1_count/p2_count` 缺省时由 `issue_groups[].issues[].priority` 派生；显式不一致统计失败；已迁入 `artifactDataContract` manifest sync | 后端生成静态需求质量 Mermaid `flowchart`；由 `quality_overview` 生成 `ai4se-visual score-matrix` | `test_req_review_artifact_data_derives_issue_statistics_counts_when_missing`、`test_req_review_artifact_data_rejects_inconsistent_issue_statistics`、`test_req_review_review_artifact_data_contract_manifest_drives_backend_instruction`、visual contract maps |
...
| `INCIDENT_REVIEW/IMPROVEMENT` | 报告基础信息、时间线摘要、根因摘要、改进行动、根因覆盖、防复发、复查计划、遗留风险、经验教训、组织学习、签署、阶段门禁 | `report_info.action_count` 缺省时由 `improvement_actions` 数量派生；`priority_distribution` 缺省时由 `improvement_actions[].priority` 派生；显式不一致统计失败；根因覆盖只校验不派生；已迁入 `artifactDataContract` manifest sync | 后端由 `priority_distribution` 生成 Mermaid `pie`；由 `improvement_actions` 生成 `ai4se-visual action-board` | `test_incident_improvement_artifact_data_derives_action_statistics_when_missing`、`test_incident_improvement_artifact_data_rejects_*`、`test_incident_improvement_artifact_data_contract_manifest_drives_backend_instruction`、visual contract maps |
...
| `IDEA_BRAINSTORM/CONVERGE` | 决策矩阵、ICE 评估、资源约束、敏感性分析、验证实验、合并路径、阶段门禁 | `ice_score` 缺省时由后端按 `impact * confidence / effort` 派生，显式错误值仍触发 validation failure；已迁入 `artifactDataContract` manifest sync | 后端由 `ice_evaluations` 生成 Mermaid `quadrantChart` | `test_idea_converge_artifact_data_derives_ice_score_when_missing`、`test_idea_converge_artifact_data_rejects_*`、`test_idea_converge_artifact_data_contract_manifest_drives_backend_instruction` |
