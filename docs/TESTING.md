...
当前矩阵不表示所有派生字段都已后端化，也不表示所有阶段都完成 `artifactDataContract` manifest 同步。当前已迁入 `artifactDataContract` manifest sync 的阶段是 `IDEA_BRAINSTORM/DEFINE`、`IDEA_BRAINSTORM/CONVERGE`、`TEST_DESIGN/CASES`、`TEST_DESIGN/STRATEGY` 和 `INCIDENT_REVIEW/ROOT_CAUSE`；其余 20 个 artifact-data 阶段的关键不变量仍主要由 Pydantic model、renderer tests、runtime instruction 和 artifact contract tests 共同保护。Mermaid 仍是后端 deterministic renderer 的编译目标；backend 仍不执行 Mermaid JS parse 或 `mmdc` 渲染门禁。
