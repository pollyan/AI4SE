...
| `TEST_DESIGN/DELIVERY` | 文档信息、交付指标、执行摘要、需求摘要、策略摘要、用例摘要、覆盖地图、开放风险、验收、签署、变更记录 | `delivery_metrics.total_cases`、`high_risk_count` 和用例摘要统计为一致性校验，不派生字段 | 后端由 `coverage_map` 生成 `ai4se-visual coverage-map` | `test_delivery_artifact_data_rejects_inconsistent_case_totals`、`REQUIRED_ARTIFACT_STRUCTURED_VISUALS` |
