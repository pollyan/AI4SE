...
| `TEST_DESIGN/DELIVERY` | 文档信息、交付指标、执行摘要、需求摘要、策略摘要、用例摘要、覆盖地图、开放风险、验收、签署、变更记录 | `delivery_metrics.total_cases`、`high_risk_count` 和用例摘要统计为一致性校验，不派生字段；已迁入 `artifactDataContract` manifest sync | 后端由 `coverage_map` 生成 `ai4se-visual coverage-map` | `test_delivery_artifact_data_rejects_inconsistent_case_totals`、`test_test_design_delivery_artifact_data_contract_manifest_drives_backend_instruction`、`REQUIRED_ARTIFACT_STRUCTURED_VISUALS` |
| `REQ_REVIEW/REVIEW` | 评审信息、评审范围、质量总览、问题统计、问题分组、修订建议、阶段门禁 | 校验问题统计一致性，不派生字段；已迁入 `artifactDataContract` manifest sync | 后端生成静态需求质量 Mermaid `flowchart`；由 `quality_overview` 生成 `ai4se-visual score-matrix` | `test_req_review_artifact_data_rejects_inconsistent_issue_statistics`、`test_req_review_review_artifact_data_contract_manifest_drives_backend_instruction`、visual contract maps |
| `REQ_REVIEW/REPORT` | 评审结论、评审信息、问题统计、关闭清单、复审条件、签署、变更记录 | 校验问题统计一致性，不派生字段；已迁入 `artifactDataContract` manifest sync | 后端由 `issue_statistics` 生成 Mermaid `pie`；由 `issue_closures` 生成 `ai4se-visual priority-board` | `test_req_review_report_artifact_data_rejects_inconsistent_issue_statistics`、`test_req_review_report_artifact_data_contract_manifest_drives_backend_instruction`、visual contract maps |
