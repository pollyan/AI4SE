...
| `STORY_BREAKDOWN/INPUT_ANALYSIS` | 文档信息、输入分析、Epic、用户故事、验收标准、依赖、Sprint 切片、Lisa handoff 输入、阶段门禁 | ID 唯一性、Epic / Story / Criterion / Dependency / Sprint / handoff 引用和 `stage_gate` checked 只校验不派生；`story_points >= 1`；已迁入 `artifactDataContract` manifest sync | 后端由 `epics` 生成 Mermaid `flowchart`；由 `user_stories` 生成 `ai4se-visual story-map` | `test_story_breakdown_artifact_data_rejects_contract_violations`、`test_story_breakdown_artifact_data_contract_manifest_drives_backend_instruction`、`REQUIRED_ARTIFACT_MERMAID_DIAGRAMS` |
| `STORY_BREAKDOWN/EPIC_MAPPING` | 文档信息、输入分析、Epic、用户故事、验收标准、依赖、Sprint 切片、Lisa handoff 输入、阶段门禁 | ID 唯一性、Epic / Story / Criterion / Dependency / Sprint / handoff 引用和 `stage_gate` checked 只校验不派生；`story_points >= 1`；已迁入 `artifactDataContract` manifest sync | 后端由 `epics` 生成 Mermaid `flowchart`；由 `user_stories` 生成 `ai4se-visual story-map` | `test_story_breakdown_artifact_data_rejects_contract_violations`、`test_story_breakdown_artifact_data_contract_manifest_drives_backend_instruction`、`test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` |
...
当前矩阵不表示所有派生字段都已后端化，也不表示所有阶段都完成 `artifactDataContract` manifest 同步。当前已迁入 `artifactDataContract` manifest sync 的阶段是 `IDEA_BRAINSTORM/DEFINE`、`IDEA_BRAINSTORM/DIVERGE`、`IDEA_BRAINSTORM/CONVERGE`、`IDEA_BRAINSTORM/CONCEPT`、`VALUE_DISCOVERY/ELEVATOR`、`VALUE_DISCOVERY/PERSONA`、`VALUE_DISCOVERY/JOURNEY`、`VALUE_DISCOVERY/BLUEPRINT`、`STORY_BREAKDOWN/INPUT_ANALYSIS`、`STORY_BREAKDOWN/EPIC_MAPPING`、`STORY_BREAKDOWN/STORY_BACKLOG`、`STORY_BREAKDOWN/SPRINT_PLAN`、`TEST_DESIGN/CLARIFY`、`TEST_DESIGN/STRATEGY`、`TEST_DESIGN/CASES`、`TEST_DESIGN/DELIVERY`、`REQ_REVIEW/REVIEW`、`REQ_REVIEW/REPORT`、`INCIDENT_REVIEW/TIMELINE`、`INCIDENT_REVIEW/ROOT_CAUSE` 和 `INCIDENT_REVIEW/IMPROVEMENT`；其余 4 个 artifact-data 阶段的关键不变量仍主要由 Pydantic model、renderer tests、runtime instruction 和 artifact contract tests 共同保护。Mermaid 仍是后端 deterministic renderer 的编译目标；backend 仍不执行 Mermaid JS parse 或 `mmdc` 渲染门禁。
