...
- 21 个 `test_runtime_raw_json_stream_turn_renders_*artifact_data*` 用例：覆盖各阶段在 final `agent_turn` 前产生正式 artifact delta；其中 `STORY_BREAKDOWN` 四阶段由 `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` 参数化覆盖，`STORY_BACKLOG` 另有 partial patch 细节测试。
...
当前矩阵不表示所有派生字段都已后端化，也不表示所有阶段都完成 `artifactDataContract` manifest 同步。当前已迁入 `artifactDataContract` manifest sync 的阶段是 `IDEA_BRAINSTORM/CONVERGE` 和 `TEST_DESIGN/CASES`；其余 19 个 artifact-data 阶段的关键不变量仍主要由 Pydantic model、renderer tests、runtime instruction 和 artifact contract tests 共同保护。Mermaid 仍是后端 deterministic renderer 的编译目标；backend 仍不执行 Mermaid JS parse 或 `mmdc` 渲染门禁。
