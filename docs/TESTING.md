...
| `INCIDENT_REVIEW/IMPROVEMENT` | 报告信息、时间线摘要、根因摘要、优先级分布、改进行动、根因覆盖、防复发、复查计划、遗留风险、经验教训、组织学习、签署、阶段门禁 | 校验行动数量、优先级分布和根因覆盖一致性，不派生字段；已迁入 `artifactDataContract` manifest sync | 后端由 `priority_distribution` 生成 Mermaid `pie`；由 `improvement_actions` 生成 `ai4se-visual action-board` | `test_incident_improvement_artifact_data_rejects_*`、`test_incident_improvement_artifact_data_contract_manifest_drives_backend_instruction`、visual contract maps |
