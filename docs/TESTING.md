...
- 25 个 `test_runtime_raw_json_stream_turn_renders_*artifact_data*` 用例：覆盖各阶段在 final `agent_turn` 前产生正式 artifact delta；其中 `STORY_BREAKDOWN` 四阶段由 `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` 参数化覆盖，`PRD_REVIEW` 四阶段由 artifact-data registry 参数化覆盖，`STORY_BACKLOG` 另有 partial patch 细节测试。
...
第 8B 轮补充 `artifact_data` 字段来源与视觉协议矩阵。该矩阵用于说明当前工程事实，不替代 Pydantic schema、renderer tests 或 `workflow_manifest.json`；维护时以 `ARTIFACT_DATA_STAGE_FIXTURES`、`REQUIRED_ARTIFACT_MERMAID_DIAGRAMS`、`REQUIRED_ARTIFACT_STRUCTURED_VISUALS`、`artifact_data_renderers.py` 和对应 renderer / runtime tests 为事实源。

| Workflow / Stage | 模型负责的 artifact_data | 后端派生 / 归一化 | 视觉来源 | 证据 |
|---|---|---|---|---|
| `TEST_DESIGN/CLARIFY` | 文档信息、需求事实、系统边界、业务规则、链路、澄清问题、质量需求、下游输入、阶段门禁 | 校验必填值和列表，不派生字段 | 后端由 `flow_links` 生成 Mermaid `flowchart` | `ClarifyArtifactData`、`test_clarify_artifact_data_rejects_*`、`REQUIRED_ARTIFACT_MERMAID_DIAGRAMS` |
| `TEST_DESIGN/STRATEGY` | 文档信息、策略摘要、质量目标、风险 S/O/D、测试技术、测试分层、测试点、取舍、阶段门禁 | `risks[].rpn` 缺省时由 S/O/D 派生；显式错误 RPN 失败；ID/引用只校验不派生 | 后端由 `risks` 生成 Mermaid `quadrantChart` 和 `ai4se-visual risk-board`；由 `test_layers` 生成 Mermaid `block-beta` | `test_strategy_artifact_data_computes_missing_rpn_for_generated_visuals`、`test_strategy_artifact_data_rejects_inconsistent_rpn`、visual contract maps |
| `TEST_DESIGN/CASES` | 文档信息、设计依据、用例分组、测试数据环境、自动化候选、覆盖追溯、开放问题、阶段门禁 | `case_statistics` 缺省时由 `case_groups` 派生；显式不一致统计失败；case_id 引用只校验不派生；已迁入 `artifactDataContract` manifest sync | 后端由 `coverage_trace` 生成 `ai4se-visual traceability-matrix` | `test_cases_artifact_data_derives_statistics_when_missing`、`test_cases_artifact_data_rejects_inconsistent_statistics`、`test_cases_artifact_data_rejects_unknown_*_case_reference`、`test_cases_artifact_data_contract_manifest_drives_backend_instruction` |
| `TEST_DESIGN/DELIVERY` | 文档信息、交付指标、执行摘要、需求摘要、策略摘要、用例摘要、覆盖地图、开放风险、验收、签署、变更记录 | `delivery_metrics.total_cases`、`high_risk_count` 和用例摘要统计为一致性校验，不派生字段 | 后端由 `coverage_map` 生成 `ai4se-visual coverage-map` 和 `traceability-matrix` | `test_delivery_artifact_data_rejects_inconsistent_case_totals`、`REQUIRED_ARTIFACT_STRUCTURED_VISUALS` |
| `REQ_REVIEW/REVIEW` | 评审信息、评审范围、质量总览、问题统计、问题分组、修订建议、阶段门禁 | 校验问题统计一致性，不派生字段 | 后端生成静态需求质量 Mermaid `flowchart`；由 `quality_overview` 生成 `ai4se-visual score-matrix` | `test_req_review_artifact_data_rejects_inconsistent_issue_statistics`、visual contract maps |
| `REQ_REVIEW/REPORT` | 评审结论、评审信息、问题统计、关闭清单、复审条件、签署、变更记录 | 校验问题统计一致性，不派生字段 | 后端由 `issue_statistics` 生成 Mermaid `pie`；由 `issue_closures` 生成 `ai4se-visual priority-board` | `test_req_review_report_artifact_data_rejects_inconsistent_issue_statistics`、visual contract maps |
| `INCIDENT_REVIEW/TIMELINE` | 事件概要、影响指标、事实来源、时间线、事实/推测隔离、事实摘要、参与人、缺失信息、阶段门禁 | fact_id 引用只校验不派生 | 后端由 `timeline_events` 生成 Mermaid `timeline` | `test_incident_timeline_artifact_data_rejects_*`、`REQUIRED_ARTIFACT_MERMAID_DIAGRAMS` |
| `INCIDENT_REVIEW/ROOT_CAUSE` | 分析上下文、5-Why、根因证据、鱼骨分类、根因结论、排除项、未验证原因、阶段门禁 | cause_id 引用和 5-Why 深度只校验不派生 | 后端由 `why_chain` 生成 `ai4se-visual cause-map`；由 `fishbone_categories` 生成 Mermaid `mindmap` | `test_render_incident_root_cause_artifact_data_is_deterministic_and_contract_valid`、`test_incident_root_cause_artifact_data_rejects_*`、cause-map contract tests |
| `INCIDENT_REVIEW/IMPROVEMENT` | 报告信息、时间线摘要、根因摘要、优先级分布、改进行动、根因覆盖、防复发、复查计划、遗留风险、经验教训、组织学习、签署、阶段门禁 | 校验行动数量、优先级分布和根因覆盖一致性，不派生字段 | 后端由 `priority_distribution` 生成 Mermaid `pie`；由 `improvement_actions` 生成 `ai4se-visual action-board` | `test_incident_improvement_artifact_data_rejects_*`、visual contract maps |
| `IDEA_BRAINSTORM/DEFINE` | 问题假设、目标用户、问题全景、证据、问题-用户匹配、约束、反向验证、阶段门禁 | problem/evidence 引用只校验不派生 | 后端由 `problem_landscape` 生成 Mermaid `mindmap` | `test_idea_define_artifact_data_rejects_*`、`test_idea_define_structured_output_instruction_explains_root_problem_coverage` |
| `IDEA_BRAINSTORM/DIVERGE` | 发散方法、创意全景、创意卡、创意来源、搁置/排除记录、阶段门禁 | idea/source 引用和唯一性只校验不派生 | 后端由 `idea_landscape` / `idea_cards` 生成 Mermaid `mindmap` | `test_idea_diverge_artifact_data_rejects_*`、partial 引用门禁 tests |
| `IDEA_BRAINSTORM/CONVERGE` | 决策矩阵、ICE 评估、资源约束、敏感性分析、验证实验、合并路径、阶段门禁 | `ice_score` 必须由模型给出并通过校验，不后端补齐；已迁入 `artifactDataContract` manifest sync | 后端由 `ice_evaluations` 生成 Mermaid `quadrantChart` | `test_idea_converge_artifact_data_rejects_*`、`test_converge_artifact_data_contract_manifest_drives_backend_instruction` |
| `IDEA_BRAINSTORM/CONCEPT` | 定位声明、核心假设、Lean Canvas、MVP 功能、增长漏斗、风险、验证路线、不可做范围、决策记录、下一步、阶段门禁 | assumption/action 引用和必备画布格只校验不派生；MVP pie 计数仅在 renderer 中计算 | manifest 只要求 `ai4se-visual mvp-map`；renderer 额外由 `mvp_features` 生成 Mermaid pie，由 `growth_funnel` 生成 Mermaid flowchart | `test_idea_concept_artifact_data_rejects_*`、`REQUIRED_ARTIFACT_STRUCTURED_VISUALS` |
| `VALUE_DISCOVERY/ELEVATOR` | 文档信息、定位摘要、价值流、目标场景、痛点证据、差异化、商业可行性、评分矩阵、评分摘要、假设、电梯演讲、阶段门禁 | `score_summary.total_score` / `average_score` 由 `score_matrix` 归一化；显式不一致摘要失败 | 后端由 `value_flow` 生成 Mermaid `flowchart`；由 `score_matrix` / `score_summary` 生成 `ai4se-visual score-matrix` | `test_value_elevator_artifact_data_computes_missing_score_summary_fields`、`test_value_elevator_artifact_data_rejects_inconsistent_score_summary` |
| `VALUE_DISCOVERY/PERSONA` | 文档信息、画像摘要、画像、行为场景、决策链、痛点证据、反画像、优先级排序、阶段门禁 | persona 引用和优先级唯一性只校验不派生 | 无 required visual；后端渲染画像、行为和决策链表格 | `test_value_persona_artifact_data_rejects_*`、`ARTIFACT_DATA_STAGE_FIXTURES` |
| `VALUE_DISCOVERY/JOURNEY` | 文档信息、旅程摘要、旅程阶段、痛点优先级、机会评分、切入策略、验证实验、阶段门禁 | stage/opportunity 引用只校验不派生 | 后端由 `journey_stages` 生成 Mermaid `journey` 和 `ai4se-visual journey-map` | `test_value_journey_artifact_data_rejects_*`、visual contract maps |
| `VALUE_DISCOVERY/BLUEPRINT` | 文档信息、产品概述、目标用户、功能模块、需求、主流程、成功指标、MVP 计划、非功能需求、验收标准、路线图、风险、Lisa handoff 输入、阶段门禁 | requirement/handoff 引用只校验不派生 | manifest 只要求 `ai4se-visual roadmap`；renderer 额外由 `feature_modules` 生成 Mermaid `mindmap`，由 `main_flow` 生成 Mermaid `flowchart` | `test_value_blueprint_artifact_data_rejects_*`、visual contract maps |
| `STORY_BREAKDOWN/INPUT_ANALYSIS` | 文档信息、输入范围、需求事实、上游追溯、不拆范围、阻塞问题、阶段门禁 | requirement trace 只校验不派生 | 后端由 `traceability_index` 生成 Mermaid `flowchart` | `test_render_story_breakdown_artifact_data_outputs_story_package`、`REQUIRED_ARTIFACT_MERMAID_DIAGRAMS` |
| `STORY_BREAKDOWN/EPIC_MAPPING` | 文档信息、需求、Epic、活动、任务、故事地图项、MVP slice、Release slice、阶段门禁 | story/activity/task 关系只校验不派生 | 后端由 `story_map_items` / `tasks` 生成 Mermaid `flowchart` | `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data`、visual contract maps |
| `STORY_BREAKDOWN/STORY_BACKLOG` | 文档信息、需求、拆分原则、故事卡、Ready 汇总、Not Ready 故事、开放问题、阶段门禁 | storyId / requirementId 引用和 Ready / Not Ready 状态只校验不派生 | 无 required visual；后端渲染用户故事卡片和 Ready / Not Ready 表格 | `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data`、story handoff tests |
| `STORY_BREAKDOWN/SPRINT_PLAN` | 文档信息、需求、Sprint 切片、Ready 总览、单故事需求包、上游追溯、Not Ready 阻塞项、AI Coding 输入边界、阶段门禁 | 单故事包 trace / boundary 只校验不派生 | `ai4se-visual story-map` | `test_render_story_breakdown_artifact_data_outputs_story_package`、single story handoff tests |
| `PRD_REVIEW/INVENTORY` | 文档信息、PRD 输入盘点、质量发现、补全动作、修订章节、验收标准、handoff 输入、阶段门禁 | finding/action/section 引用和 stage gate 只校验不派生 | 后端由 `prd_inventory` 生成 Mermaid `mindmap` | `test_render_prd_review_artifact_data_is_deterministic_and_contract_valid`、`ARTIFACT_DATA_STAGE_FIXTURES` |
| `PRD_REVIEW/QUALITY_AUDIT` | 文档信息、PRD 输入盘点、质量发现、补全动作、修订章节、验收标准、handoff 输入、阶段门禁 | finding/action/section 引用和 stage gate 只校验不派生 | 后端由 `quality_findings` 生成 `ai4se-visual score-matrix` | `test_render_prd_review_artifact_data_is_deterministic_and_contract_valid`、visual contract maps |
| `PRD_REVIEW/COMPLETION_PLAN` | 文档信息、PRD 输入盘点、质量发现、补全动作、修订章节、验收标准、handoff 输入、阶段门禁 | finding/action/section 引用和 stage gate 只校验不派生 | 后端由 `completion_actions` 生成 `ai4se-visual action-board` | `test_render_prd_review_artifact_data_is_deterministic_and_contract_valid`、visual contract maps |
| `PRD_REVIEW/REVISION_BLUEPRINT` | 文档信息、PRD 输入盘点、质量发现、补全动作、修订章节、验收标准、handoff 输入、阶段门禁 | finding/action/section 引用和 stage gate 只校验不派生 | 后端由 `revision_sections` 生成 `ai4se-visual roadmap` | `test_render_prd_review_artifact_data_is_deterministic_and_contract_valid`、visual contract maps |

当前矩阵不表示所有派生字段都已后端化，也不表示所有阶段都完成 `artifactDataContract` manifest 同步。当前已迁入 `artifactDataContract` manifest sync 的阶段是 `IDEA_BRAINSTORM/CONVERGE` 和 `TEST_DESIGN/CASES`；其余 23 个 artifact-data 阶段的关键不变量仍主要由 Pydantic model、renderer tests、runtime instruction 和 artifact contract tests 共同保护。Mermaid 仍是后端 deterministic renderer 的编译目标；backend 仍不执行 Mermaid JS parse 或 `mmdc` 渲染门禁。

架构边界：流式机制是统一的，不为 Lisa、Alex、IDEA、VALUE 或后续 workflow 新增独立 runtime、SSE endpoint、前端 store 或 ArtifactPane 渲染管线。阶段差异只允许体现在 `workflow_manifest.json`、stage prompt、artifact_data schema、partial renderer 和 contract 测试里；每新增一个在线阶段，都必须进入下面的 25 阶段同类矩阵并补齐指令顺序、partial renderer、runtime raw JSON streaming 和最终 contract 验证。
...
| `STORY_BREAKDOWN/INPUT_ANALYSIS` | `test_render_story_breakdown_artifact_data_outputs_story_package` 覆盖 final renderer | `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` | 需求范围、需求追溯、阻塞问题、Mermaid flowchart |
| `STORY_BREAKDOWN/EPIC_MAPPING` | `test_render_story_breakdown_artifact_data_outputs_story_package` 覆盖 final renderer | `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` | Epic / Story Map、MVP slice、Mermaid flowchart |
| `STORY_BREAKDOWN/STORY_BACKLOG` | `test_render_story_breakdown_artifact_data_outputs_story_package` 覆盖 final renderer | `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` | 用户故事卡片、Ready / Not Ready 质量校验、storyId / requirementId 引用 |
| `STORY_BREAKDOWN/SPRINT_PLAN` | `test_render_story_breakdown_artifact_data_outputs_story_package` 覆盖 final renderer | `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` | Sprint 切片、Ready story handoff 清单、单故事需求包 JSON、AI Coding 输入边界 |
| `PRD_REVIEW/INVENTORY` | `test_render_prd_review_artifact_data_is_deterministic_and_contract_valid` 覆盖 final renderer | artifact-data registry 参数化 raw JSON streaming 覆盖 | PRD 输入盘点、Mermaid mindmap、stage gate |
| `PRD_REVIEW/QUALITY_AUDIT` | `test_render_prd_review_artifact_data_is_deterministic_and_contract_valid` 覆盖 final renderer | artifact-data registry 参数化 raw JSON streaming 覆盖 | PRD 质量评审、`score-matrix`、finding 引用 |
| `PRD_REVIEW/COMPLETION_PLAN` | `test_render_prd_review_artifact_data_is_deterministic_and_contract_valid` 覆盖 final renderer | artifact-data registry 参数化 raw JSON streaming 覆盖 | PRD 补全建议、`action-board`、action/finding 引用 |
| `PRD_REVIEW/REVISION_BLUEPRINT` | `test_render_prd_review_artifact_data_is_deterministic_and_contract_valid` 覆盖 final renderer | artifact-data registry 参数化 raw JSON streaming 覆盖 | PRD 修订蓝图、`roadmap`、验收标准和 handoff 输入 |
