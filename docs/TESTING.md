...
- 各工作流阶段的必需标题必须完整出现。
- 关键阶段的可视化 contract 必须完整出现，例如 `REQUIRED_ARTIFACT_MERMAID_DIAGRAMS` 要求的 `quadrantChart`、`mindmap`、`journey`，以及 `REQUIRED_ARTIFACT_STRUCTURED_VISUALS` 要求的 `timeline-map`、`cause-map`、`story-map` 等。
