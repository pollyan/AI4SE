...
| `INCIDENT_REVIEW/TIMELINE` | 暂未实现；完整 artifact_data 到齐时 before-final delta | `test_runtime_raw_json_stream_turn_renders_incident_timeline_artifact_data_before_final_output` | `timeline-map`、事实来源与时间线 contract |
...
| `STORY_BREAKDOWN/INPUT_ANALYSIS` | 暂未实现；完整 artifact_data 到齐时 before-final delta | `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` | 需求范围、需求追溯、阻塞问题、`flow-map` |
| `STORY_BREAKDOWN/EPIC_MAPPING` | 暂未实现；完整 artifact_data 到齐时 before-final delta | `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` | Epic / Story Map、MVP slice、`flow-map` |
| `STORY_BREAKDOWN/STORY_BACKLOG` | 暂未实现；完整 artifact_data 到齐时 before-final delta | `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` | 用户故事卡片、Ready / Not Ready 质量校验、storyId / requirementId 引用、`flow-map` |
| `STORY_BREAKDOWN/SPRINT_PLAN` | 暂未实现；完整 artifact_data 到齐时 before-final delta | `test_runtime_raw_json_stream_turn_renders_all_story_breakdown_stages_from_artifact_data` | Sprint 切片、Ready story handoff 清单、单故事需求包 JSON、AI Coding 输入边界、`flow-map` / `story-map` |
...
New Agents 另有一套独立于 intent-tester/MidScene 的浏览器级工作流测试，位于 `tests/e2e/new_agents_browser/`。它使用 Python Playwright 打开真实 React 前端，通过拆分的 mock typed SSE 响应验证 Lisa `test-design` 和 Alex `value-discovery` 的完整阶段组织逻辑。runner 在发送前安装 `MutationObserver`，通过 `chat-pane`、`assistant-message-content` 与 `artifact-content` 语义定位记录首屏顺序；同一浏览器提交中同时出现会显式记为 `simultaneous` 并失败。`WorkflowRunResult.stream_order` 必须为 `("chat", "artifact")`。其余结构化结果包括 `final_artifact`、`stage_artifacts`、`conversation_events` 和 `stage_transitions`，供确定性断言和可选 LLM judge 共用。
