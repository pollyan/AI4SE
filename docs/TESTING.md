...
| `TEST_DESIGN/CASES` | 文档信息、设计依据、用例分组、测试数据环境、自动化候选、覆盖追溯、开放问题、阶段门禁 | `case_statistics` 缺省时由 `case_groups` 派生；显式不一致统计失败；case_id 引用只校验不派生 | 后端由 `coverage_trace` 生成 `ai4se-visual traceability-matrix` | `test_cases_artifact_data_derives_statistics_when_missing`、`test_cases_artifact_data_rejects_inconsistent_statistics`、`test_cases_artifact_data_rejects_unknown_*_case_reference` |
...
| `IDEA_BRAINSTORM/CONVERGE` | 决策矩阵、ICE 评估、资源约束、敏感性分析、验证实验、合并路径、阶段门禁 | `ice_score` 必须由模型给出并通过校验，不后端补齐；当前唯一已迁入 `artifactDataContract` manifest sync 的阶段 | 后端由 `ice_evaluations` 生成 Mermaid `quadrantChart` | `test_idea_converge_artifact_data_rejects_*`、`test_converge_artifact_data_contract_manifest_drives_backend_instruction` |
