...
每个能力包都必须作为完整厚切片推进，默认包含 CGA、中文 spec、中文 implementation plan、TDD、验证、todo 更新和聚焦 commit。不要把同一能力包拆成“先 schema、再 UI、再测试、再文档”的多轮小切片；同一用户意图下的后端契约、前端入口、状态承接、错误反馈、验证证据和文档记录应在同一轮交付。DeepSeek 真实 smoke 是例外：如果当轮缺少凭证、网络或额度，只能交付真实 smoke gate / 证据记录机制，并明确真实外部验证未完成，不能用 mock 结果替代真实 smoke。
