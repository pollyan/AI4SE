...
- `2026-06-23-new-agents-enhancement-diagnostic.md`：New Agents 功能盘点、差距分析和增强路线活动候选；已记录 2026-06-23 Alex `PRD_REVIEW` 质量评审与补全 workflow、`STORY_BREAKDOWN` 用户故事拆解 workflow、Artifact 审阅诊断中心、Lisa 测试资产质量闭环、Workflow handoff 上下文审阅、Run 历史复用中心、DeepSeek 格式化失败运行统计诊断闭环、Workflow 质量治理闭环、Artifact 定向修订闭环和 Workflow schema dry-run 门禁消化结果，下一批高价值候选为 E08 跨 run 质量趋势/LLM judge、E12 完整 scaffold/codegen、prompt/template 版本管理和 DeepSeek 真实 smoke。
