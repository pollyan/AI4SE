...
本节是后续目标模式读取本文件时的活动入口。下方 P0/P1/P2 条目保留完整历史进展；较早记录里的“剩余”可能已被后续 CGA、产品决策或当前活动状态覆盖，不能直接当作当前待办进入实现。

| 能力包 | 当前状态 | 证据 | 后续处理 |
| --- | --- | --- | --- |
| 工作区顶部操作收敛 | 阶段完成 | 已完成 Header 一级操作收敛、低频入口二级化、Artifact 工具条二级操作收敛和无真实进度条清理 | 仅在出现新的顶部操作混乱证据时重启 CGA |
| 结构化输出失败与重试体验 | 阶段完成 | 已完成失败恢复卡片、重试本阶段生成、连续失败补充信息、可视化运行时诊断和定位右侧问题位置 | 仅在出现新的失败恢复断点或阶段推进误导证据时重启 CGA |
| 全流程专业产出物与可视化增强 | 阶段完成 | 已完成首阶段可视化契约、后续关键阶段结构化可视化、剩余关键阶段补强、E2E 与 LLM judge 可视化质量证据 | 高保真导出不再作为当前活动项；如重启必须重新证明用户价值 |
| Mermaid 可靠性与结构化可视化扩展 | 阶段完成 | 已完成多类 `ai4se-visual`、Mermaid fence 模板修复、生产重复渲染缓存、策略 Mermaid sanitizer 兼容 | 仅按生产事故、新高频失败类型或完整可视化稳定性闭环重启 |
| 左侧对话自然表达与重点可扫描 | 阶段完成 | 已完成自然顾问式对话契约，保留聊天自由度，不引入固定字段化 chat schema | 不按“语义化摘要固定模板”继续推进；如重启需证明自然表达仍不可读 |
| 模型配置与供应商治理融入体验 | 阶段完成 | 已完成供应商失败归因、失败卡片动作、运行统计问题可见性、pre-runtime 观测、告警动作、首次模型配置自检与修复闭环 | 仅按新的供应商治理主路径缺口重启，不按单个错误码拆薄 |
| Artifact 协作体验深化 | 当前阶段完成 | 已完成受控编辑、服务端保存、冲突保护、版本对比/刷新、导出水位、章节锁定、批注、审阅面板、批注锚点、块级处理、大量安全自动合并和冲突安全边界收口 | 仅在出现完整且可证明安全的用户冲突场景时重启；歧义场景保持人工处理 |

## 当前明确不推进

- 高保真 PDF 图片级导出继续深化。当前 Word/DOCX 包级导出、Markdown/PDF 语义投影和 DOCX Mermaid SVG 嵌入已达到本阶段可接受水位。
- 恢复中心。当前通过 artifact history、冲突对比、刷新、审阅面板和批注锚点处理承接主路径。
- 分享/权限。
- 多人实时协同。
- 与 intent-tester 自动打通。
- 单个 Mermaid 语法变体、单个导出格式、单个三方 merge 算法分支，除非能聚合为完整用户功能厚切片或独立工程信任闭环。

## 后续 CGA 选题规则

- 历史进展记录中的“剩余”是当时的过程记录，不自动等同于当前活动待办。
- 后续目标模式必须以上方当前活动状态、用户最新决策和当前代码证据为准。
- 不能从单个旧“剩余”、单个导出格式、单个 Mermaid 语法变体、单个模型供应商错误码或单个三方 merge 算法分支直接进入 milestone。
- 如果某个旧剩余项看起来仍有价值，必须先重新聚合成完整用户功能厚切片，覆盖入口、动作、系统处理、可见结果、状态承接、失败反馈和证据。
- 如果无法聚合成完整能力包，或用户已明确不推进，应在 CGA 中标记为阶段完成、暂不推进或等待新证据，而不是继续实现。

## 历史进展记录说明

以下 P0/P1/P2 条目保留完整过程记录。较早记录里的“剩余”可能已被后续 CGA、产品决策或本文件顶部的当前活动状态覆盖；后续选题以本文件顶部的当前活动状态为准。
...
- 2026-06-20：完成第三十二块 CGA「Artifact DOCX 可视化语义投影」。
  ...
  - 剩余：更复杂的改写/移动语义自动合并、复杂 Mermaid/SVG 高保真嵌入仍可作为后续增强切片。
- 2026-06-20：完成第三十三块 CGA「Artifact PDF Mermaid Timeline 矢量投影」。
  - PDF 导出现在会识别 Mermaid `timeline`，清洗 `title` / `section` 控制行，保留标题、分段、时间点和事件描述为可搜索文本。
  - PDF content stream 会绘制轻量时间轴主线、刻度线、事件连接线和事件框，让故障复盘时间线不再只是源码文本摘要。
  - 实现继续复用共享 Artifact PDF 导出路径，不新增 Incident Review、Lisa/Alex 或 workflow 专属分支；复杂或不支持的 Mermaid 仍按文本摘要降级，不阻断下载。
  - 验证：先运行 `npm run test -- --run src/components/__tests__/ArtifactPane.test.tsx -t "draws Mermaid timelines"` 观察到 timeline 仍输出 `title`/`section` 控制行且缺少图形命令失败；实现后同命令通过；`npm run test -- --run src/components/__tests__/ArtifactPane.test.tsx`。
  - 剩余：更复杂的改写/移动语义自动合并、mindmap/journey/pie 等复杂 Mermaid/SVG 高保真嵌入仍可作为后续增强切片。
- 2026-06-20：完成第三十四块 CGA「Artifact PDF Mermaid Mindmap 矢量投影」。
  - PDF 导出现在会识别 Mermaid `mindmap`，清洗 `root((...))`、`[...]`、`(...)` 等常见节点包装，保留根节点和子节点为可搜索文本。
  - PDF content stream 会绘制轻量树状节点框和父子连接线，让创意问题树、根因鱼骨图导出后不再只是 Mermaid 源码摘要。
...
- 2026-06-20：完成第五十块 CGA「Artifact fenced block 内部行重排自动合并」。
  - 保存冲突现在会识别保守 fenced code / Mermaid block 内部行重排：一侧只调整同一个 fenced block 内部行顺序，开闭 fence、块外内容、行文本和行集合完全不变，另一侧只改写其他普通章节时，可使用 `自动合并非重叠变更`。
  - 点击后编辑草稿会同时保留安全的代码块内部顺序和另一侧章节改写，并记录 `artifact_auto_merge_applied`，summary 区分 `非重叠代码块行重排`。
...
- 2026-06-20：完成第五十六块 CGA「Artifact 批注锚点失效提示」。
  ...
  - 失效提示说明 `正文已变化，请重新确认这条批注的位置。`，避免用户点击定位没有反馈。
...
- 2026-06-20：完成第五十八块 CGA「Artifact 冲突处理收尾能力包」。
  - 保存冲突现在会识别同一章节内的普通段落插入与单段非重叠改写：一侧新增普通段落，另一侧改写不同旧段落时，可使用 `自动合并非重叠变更`。
  - 点击后编辑草稿会同时保留安全新增段落和另一侧非冲突段落改写，并记录 `artifact_auto_merge_applied`，summary 区分 `同章节非重叠段落插入与改写`。
  - 如果插入与改写形态涉及同一章节多处段落、顺序无法证明或可能覆盖用户/服务端有效修改，系统会拦截宽松 insertion fallback，不显示自动合并入口。
