...
## 改造方向（历史设计，当前 P0/P1 已完成）

### 已完成 P0: 引入阶段产物数据 schema
...
### 已完成 P0: 后端确定性 renderer

已新增共享 renderer，把 `artifact_data` 渲染为当前前端可消费的 Markdown Artifact。
...
### 已完成 P0: DeepSeek V4 JSON mode adapter 收束

已把 DeepSeek V4 Flash 作为明确 provider capability tier: `json_object_only`。
...
### 已完成 P1: 纠错重试从 Markdown 纠错改为数据纠错

失败反馈已围绕 `artifact_data` schema 和业务 contract，而不是让模型重写整篇 Markdown。
