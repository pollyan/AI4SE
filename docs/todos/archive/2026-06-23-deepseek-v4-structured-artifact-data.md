...
- `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 /Users/anhui/Documents/myProgram/AI4SE/.venv/bin/python -m pytest -p no:cacheprovider -o addopts='' tests/test_refactor_todo_index.py -q`：先 RED，归档 DeepSeek todo 并更新 README 后 GREEN，`1 passed`。
...
- `cd tools/new-agents/frontend && npm run test -- --run src/core/prompts/__tests__/buildSystemPrompt.test.ts`：`40 passed`。
- `cd tools/new-agents/frontend && npm run lint`：通过，执行 `tsc --noEmit`。
