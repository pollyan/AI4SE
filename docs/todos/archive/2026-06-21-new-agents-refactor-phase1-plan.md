...
2. 目标模式第 2 轮：Handoff 与 typed SSE 协议护栏闭环。（已完成，2026-06-21）
3. 目标模式第 3 轮：Workspace 状态恢复与承接稳定闭环。（已完成，2026-06-21）
...
- [x] **Step 1: 为 handoff template metadata 写失败测试**
...
- [x] **Step 2: 为 handoff prompt 行为写失败测试**
...
- [x] **Step 3: 实现 handoff template resolution**
...
- RED：新增 manifest handoff `promptTemplateId` 测试后失败，确认现有 handoff 未声明 template id。
- RED：新增 handoff prompt 断言后失败，确认 prompt 文案写死 `Alex 产出的需求蓝图`。
- GREEN：在 manifest handoff 中加入 `promptTemplateId: source-artifact-handoff`，并在 `workflow_handoffs.py` 中通过 `HANDOFF_PROMPT_TEMPLATES` 解析通用上游产物 handoff prompt。
- SSE fixture：新增 `tools/new-agents/contract-fixtures/agent-runtime-events.json`，覆盖 `run_started`、`agent_delta`、`agent_turn`、`error`；后端 encoder 和前端 parser 使用同一 fixture 语义。
- 验证：
  - `/Users/anhui/Documents/myProgram/AI4SE/.venv/bin/python -m pytest tools/new-agents/backend/tests/test_workflow_contract_sync.py tools/new-agents/backend/tests/test_workflow_handoffs.py tools/new-agents/backend/tests/test_sse_encoder.py -q`
  - `cd tools/new-agents/frontend && npm run test -- --run src/core/__tests__/llm.test.ts`
...
- [x] **Step 1: 锁定 snapshot restore 行为**
...
- [x] **Step 2: 锁定 persisted workspace sanitize 行为**
...
- RED：新增 `workspaceState` helper 测试后失败，确认 helper 模块尚不存在。
- GREEN：新增 `frontend/src/core/workspaceState.ts`，抽取 `isRecord`、`isWorkflowType`、`sanitizeCurrentRunId`、`sanitizeStageArtifacts`、`sanitizeAttachments`，并让 `store.ts` 复用这些 pure helpers。
- 行为边界：未改变 `useStore` public actions、persisted storage key、旧 `lisa-storage` 迁移逻辑、runtime API、snapshot shape 或 UI 组件。
- 验证：
  - `cd tools/new-agents/frontend && npm run test -- --run src/core/__tests__/workspaceState.test.ts src/__tests__/store.test.ts`
...
阶段 1 聚合验证已完成于 2026-06-21：

- `/Users/anhui/Documents/myProgram/AI4SE/.venv/bin/python -m pytest tools/new-agents/backend/tests/test_workflow_contract_sync.py tools/new-agents/backend/tests/test_workflow_handoffs.py tools/new-agents/backend/tests/test_sse_encoder.py tools/new-agents/backend/tests/test_request_schemas.py tools/new-agents/backend/tests/test_agent_contracts.py -q`：128 passed。
- `cd tools/new-agents/frontend && npm run test -- --run src/core/config/__tests__/workflows.test.ts src/core/__tests__/llm.test.ts src/__tests__/store.test.ts src/core/__tests__/workspaceState.test.ts src/services/__tests__/chatService.test.ts`：5 files passed，186 tests passed。
- `git diff --check`：通过。

本阶段没有修改 prompt/artifact 质量规则或用户主流程输出，因此未运行浏览器 E2E 与 LLM judge。
