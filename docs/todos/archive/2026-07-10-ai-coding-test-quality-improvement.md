...
- 状态：`ARCHIVED` — `QS-01` 至 `QS-04` 的完成证据保留；2026-07-16 用户取消 `QS-05` 至 `QS-08` 及其他旧 backlog；`QG-017` 至 `QG-019` 已迁入新的唯一活跃 Todo
- 创建日期：2026-07-10
...
- 归档边界：`QS-01` 至 `QS-04` 的实现与证据见下方执行记录；未完成旧项均按用户决定取消，不再恢复；`QG-017` 至 `QG-019` 的当前定义以 [`2026-07-16-new-agents-streaming-and-artifact-ux.md`](../2026-07-16-new-agents-streaming-and-artifact-ux.md) 为准
...
2026-07-16 用户明确要求只保留刚积累的 3 个待办。`QS-01` 至 `QS-04` 的既有交付继续作为历史证据；`QS-05` 至 `QS-08`、其承接的未完成发现和本文件内其他 P2/P3 条件触发项全部取消，不再作为恢复候选。`QG-017`、`QG-018`、`QG-019` 已迁入 [`2026-07-16-new-agents-streaming-and-artifact-ux.md`](../2026-07-16-new-agents-streaming-and-artifact-ux.md)，本文件完成归档并退出活跃入口。未来出现新失败时必须按当前事实重新建项，不得从本归档恢复旧序列。
