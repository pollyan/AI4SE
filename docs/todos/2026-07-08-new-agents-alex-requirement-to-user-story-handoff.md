...
| 阶段 | 候选 ID | 目标 | 主要产出 |
| --- | --- | --- | --- |
| 需求输入分析 | `INPUT_ANALYSIS` | 确认蓝图中哪些需求适合进入故事拆分，哪些仍被开放问题阻塞。 | 拆分范围、不拆范围、阻塞问题、需求追溯索引。 |
| 用户故事地图 | `EPIC_MAPPING` | 按 Epic、用户活动、任务和流程组织需求，识别 MVP / Release slice。 | 用户活动主干、任务步骤、候选故事、MVP slice、后续 release slice。 |
| 用户故事清单 | `STORY_BACKLOG` | 将候选故事拆成满足垂直业务切片和 INVEST 倾向的细粒度故事卡。 | 故事卡列表、验收标准、业务规则、依赖、不做范围、Ready / Not ready 分类。 |
| Sprint 与需求包 | `SPRINT_PLAN` | 标记可交给 AI Coding 的 ready stories，并生成单故事需求包。 | Ready stories、Not ready stories、单故事 handoff packet、开放问题。 |
...
- `STORY_BREAKDOWN` 已上线为 Alex 在线工作流，四个阶段已进入结构化 `artifact_data` contract，后端负责确定性渲染 Markdown / Mermaid。
- `VALUE_DISCOVERY/BLUEPRINT -> STORY_BREAKDOWN/INPUT_ANALYSIS` handoff 已接入，目标侧启动入口已覆盖 `VALUE_DISCOVERY/ELEVATOR` 和 `STORY_BREAKDOWN/INPUT_ANALYSIS`；后续 workflow 级 handoff 关系仍可升级为更强的一等持久化对象，但单故事需求包已具备独立持久化记录。
...
- 已有浏览器级 mock E2E 证明 `STORY_BREAKDOWN` 四阶段可推进、单故事 packet 可生成 / 复制，并覆盖 `VALUE_DISCOVERY/BLUEPRINT -> STORY_BREAKDOWN/INPUT_ANALYSIS -> 单故事 packet` 链路证据。
...
| 第 2 轮 | Alex 可把需求蓝图拆成完整用户故事文档 | 新增 `STORY_BREAKDOWN` 工作流，用户可从 `VALUE_DISCOVERY/BLUEPRINT` 进入完整的 `INPUT_ANALYSIS -> EPIC_MAPPING -> STORY_BACKLOG -> SPRINT_PLAN` 阶段链路，得到包含稳定 ID 和垂直业务切片的用户故事拆解文档。 |
...
- `STORY_BREAKDOWN` 出现在 Alex 工作流列表中。
- 工作流阶段为 `INPUT_ANALYSIS -> EPIC_MAPPING -> STORY_BACKLOG -> SPRINT_PLAN`。
...
- E2E 或等价浏览器测试覆盖 `VALUE_DISCOVERY/BLUEPRINT -> STORY_BREAKDOWN -> 单故事 handoff packet`。
...
- 第 2 轮应新增 `STORY_BREAKDOWN` 工作流，并从 `VALUE_DISCOVERY/BLUEPRINT` handoff 到 `INPUT_ANALYSIS -> EPIC_MAPPING -> STORY_BACKLOG -> SPRINT_PLAN`。
...
- `workflow_manifest.json` 新增 `value-discovery-blueprint-to-user-story-breakdown`，声明 `VALUE_DISCOVERY/BLUEPRINT -> STORY_BREAKDOWN/INPUT_ANALYSIS` handoff；既有 Lisa handoff 保留。
- 新增 Alex 在线工作流 `STORY_BREAKDOWN`，用户可见名为“用户故事拆解”，阶段为 `INPUT_ANALYSIS -> EPIC_MAPPING -> STORY_BACKLOG -> SPRINT_PLAN`。
- 后端 `WORKFLOW_STAGES`、artifact required headings 和 Mermaid contract 已同步新增用户故事拆解四阶段；`INPUT_ANALYSIS` 和 `EPIC_MAPPING` 要求 Mermaid `flowchart`。
...
- `ChatPane` 目标侧 handoff 启动入口已从写死 `VALUE_DISCOVERY/ELEVATOR` 扩展为配置化 copy，支持 `STORY_BREAKDOWN/INPUT_ANALYSIS` 空会话选择“开启新话题 / 从需求蓝图继续拆用户故事”。
...
结果：修复前 `5 failed, 18 passed`，失败原因是 `STORY_BREAKDOWN` workflow / handoff / stage / prompt 尚未接入。
...
- `STORY_BREAKDOWN/INPUT_ANALYSIS`、`EPIC_MAPPING`、`STORY_BACKLOG`、`SPRINT_PLAN` 已接入共享 `artifact_data` structured output 指令，模型必须先输出结构化需求数据，再输出 `chat`、`stage_action` 和 `warnings`。
...
- `INPUT_ANALYSIS` 和 `EPIC_MAPPING` 由结构化 requirement / activity / task / slice 数据确定性生成 Mermaid `flowchart`；`STORY_BACKLOG` 和 `SPRINT_PLAN` 由结构化故事卡、Ready / Not Ready 清单和单故事需求包数据确定性生成表格与 JSON 摘要。
...
- `docs/TESTING.md` 当时已扩展 partial artifact streaming / raw JSON streaming 矩阵；当前矩阵已扩展到 25 个阶段，并补充 artifactData 持久化测试责任。
- 本轮设计与执行要点已压缩保留在本文件的本轮记录中；独立过程 spec/plan 文件已清理。
...
结果：修复前失败，失败原因是 `STORY_BREAKDOWN` 尚未配置 artifact-data renderer / structured output instruction。
...
- 新增后端 story packet 服务，从 `STORY_BREAKDOWN/SPRINT_PLAN` 当前 artifact version 的结构化 `artifact_data` 读取 ready story，生成包含 source run / workflow / stage / artifact version / artifact digest / storyId / requirementIds 追溯的单故事需求包。
...
- `ArtifactPane` 在 `STORY_BREAKDOWN/SPRINT_PLAN` 预览态展示“单故事需求包”，用户可对 ready story 生成持久化需求包、看到 stale 提示，并复制结构化 JSON。
- 浏览器级 mock E2E 已覆盖用户完成 `STORY_BREAKDOWN` 四阶段后生成并复制 `US-001` 单故事需求包，且断言复制内容不包含实现计划。
...
- 新增浏览器级链路测试，从 Alex `需求蓝图梳理` 完成 `VALUE_DISCOVERY/BLUEPRINT`，在同一 workspace 点击“从需求蓝图继续拆用户故事”，恢复 `STORY_BREAKDOWN/INPUT_ANALYSIS` 目标 run。
...
- 链路测试继续完成 `STORY_BREAKDOWN` 四阶段，生成并复制 `US-001` 单故事需求包。
