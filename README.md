...
- **后端**：Flask + OpenAI SDK + SSE 流式代理

它支持多智能体、多工作流、多阶段输出，当前包含像 Lisa、Alex 这样的角色设定，以及测试设计、需求评审、头脑风暴、价值发现等工作流。

这个模块更像一个 **AI 协作工作台**，而不是一个单纯聊天页面。它关注的是：

- 如何组织阶段式工作流
- 如何管理对话产出物
- 如何在前端直连和后端代理之间切换 LLM 调用方式
...
| AI 集成 | OpenAI Python SDK, OpenAI JavaScript SDK |
