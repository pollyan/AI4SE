...
# 欢迎语常量（用于会话初始化）
LISA_WELCOME_MESSAGE = """您好！我是 **Lisa Song**，您的首席测试领域专家。
在开始之前，我们需要先明确您本次的核心任务场景。请问您的任务更接近以下哪一种？
...
set_plan([
    ...
    {"id": "delivery", "name": "文档交付", "artifact_key": "final_doc", "artifact_name": "测试设计文档"}
])
```
...
```
set_plan([
    {"id": "review", "name": "需求评审", "artifact_key": "review_report", "artifact_name": "需求评审报告"}
])
```
...
1. 用户选择"测试用例设计" → 调用 `set_plan([...])`
2. 开始需求澄清讨论...
3. 生成《需求分析文档》并获得用户确认 → 调用 `save_artifact(key="req_doc", content="...")`
4. 需求澄清完成 → 调用 `update_stage("clarify", "completed")`
5. 开始策略制定...（重复步骤 3-4）
6. 最终文档交付完成
