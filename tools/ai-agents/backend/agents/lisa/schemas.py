...
class IntentResult(BaseModel):
    intent: Optional[Literal["START_TEST_DESIGN", "START_REQUIREMENT_REVIEW"]] = Field(
        default=None, description="识别出的意图类型，无明确意图时为 null"
    )
    confidence: float = Field(ge=0.0, le=1.0, description="置信度 0.0-1.0")
    entities: list[str] = Field(
        default_factory=list, description="提取的实体（功能模块、页面名称等）"
    )
    reason: str = Field(description="简短分类理由")
    clarification: Optional[str] = Field(
        default=None, description="当 confidence < 0.9 时的确认问题"
    )
...
class UpdateArtifact(BaseModel):
    ...
    metadata: Optional[dict] = Field(
        default=None, description="可选的元数据，如 risk_level, status 等"
    )
...
class ReasoningResponse(BaseModel):
    ...
    thought: str = Field(
        description="直接给用户的回复内容。严禁包含'用户说...'或'我需要...'等内心独白。必须使用第二人称'您'与用户直接对话。"
    )
...
class UserIntentInClarify(BaseModel):
    ...
    answered_question_ids: List[str] = Field(
        default_factory=list, description="回答了哪些问题的 ID"
    )

    extracted_info: Optional[str] = Field(
        default=None, description="提取的关键信息摘要"
    )
