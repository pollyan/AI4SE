...
## 产出物输出规范 (Critical)
你必须在每次回复的最后，根据当前的对话进展，全量更新当前阶段的产出物。
请务必使用以下格式输出，**不要**在产出物代码块前后包裹任何额外的解释文字：

```markdown
[此处为更新后的产出物全量内容，保持 Markdown 格式]
```

注意：
1. **只输出一次**：产出物代码块只能出现一次，不要重复输出。
2. 必须使用 `markdown` 语言标识。
3. 必须是**全量更新**，不要只输出差异。
4. 如果当前产出物没有显著变化（例如只是简单的问候），也请输出当前的产出物内容，保持状态一致。
"""
