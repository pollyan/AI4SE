...
class SystemMonitor:
    ...
    def get_error_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        with self._lock:
            recent_errors = [
                error for error in self.error_history if error.last_occurrence >= cutoff
            ]
            ...
            summary["top_endpoints"] = sorted(
                summary["top_endpoints"].items(), key=lambda x: x[1], reverse=True
            )[:10]
    ...
    def get_performance_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        with self._lock:
            recent_metrics = [
                metric for metric in self.response_times if metric.timestamp >= cutoff
            ]
            ...
            memory_usages = [
                m.memory_usage for m in recent_metrics if m.memory_usage > 0
            ]
            ...
            for metric in recent_metrics:
                endpoint_times[f"{metric.method} {metric.endpoint}"].append(
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(
                check.get("status") == "error" for check in health["checks"].values()
            ):
                health["status"] = "error"
    ...
    def _calculate_error_rate(self, hours: int) -> float:
        ...
        with self._lock:
            total_requests = len(
                [m for m in self.response_times if m.timestamp >= cutoff]
            )
            error_requests = len(
                [
                    m
                    for m in self.response_times
    ...
    def _calculate_avg_response_time(self, hours: int) -> float:
        ...
        with self._lock:
            recent_times = [
                m.response_time for m in self.response_times if m.timestamp >= cutoff
            ]
