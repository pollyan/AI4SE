...
class AIStepExecutor:
    ...
    def __init__(self, midscene_client=None, mock_mode: bool = False):
        ...
        self.data_extractor = MidSceneDataExtractor(
            midscene_client=midscene_client, mock_mode=mock_mode
        )
        self.logger = logger
    ...
    async def execute_step(
        ...
        try:
            ...
            # 处理参数中的变量引用，支持深度递归解析
            params = self._process_variable_references(
                step_config.get("params", {}), variable_manager, step_index
            )
        ...
        try:
            ...
            if output_variable and extraction_result.success:
                ...
                if success:
                    ...
                    logger.info(
                        f"变量存储成功: {output_variable} = {extraction_result.data}"
                    )
                else:
        ...
        try:
            ...
            if action == "set_variable":
                ...
                return StepExecutionResult(
                    ...
                    return_value=var_value,
                    error_message=(
                        f"变量不存在: {var_name}" if var_value is None else None
                    ),
                )

            # 其他操作需要MidScene客户端（除了evaluateJavaScript在Mock模式下）
            if not self.midscene_client and not (
                action == "evaluateJavaScript" and self.mock_mode
            ):
                return StepExecutionResult(
                    success=False,
                    step_index=step_index,
            ...
            if action == "navigate" or action == "goto":
                ...
                return_value = await asyncio.to_thread(
                    self.midscene_client.ai_input, text, locate
                )
                ...
                return_value = await asyncio.to_thread(
                    self.midscene_client.ai_tap, prompt
                )
                ...
                return_value = await asyncio.to_thread(
                    self.midscene_client.ai_assert, prompt
                )
                ...
                if self.mock_mode:
                    ...
                    if hasattr(self.midscene_client, "evaluate"):
                        return_value = await asyncio.to_thread(
                            self.midscene_client.evaluate, script
                        )
                    elif hasattr(self.midscene_client, "page") and hasattr(
                        self.midscene_client.page, "evaluate"
                    ):
                        return_value = await asyncio.to_thread(
                            self.midscene_client.page.evaluate, script
                        )
                    else:
                ...
                try:
                    return_value = DataValidator.validate_evaluate_javascript_result(
                        return_value
                    )
    ...
    def _process_variable_references(
        ...
        try:
            ...
            logger.debug(
                f"变量引用解析完成 [步骤 {step_index}]: {params} -> {resolved_params}"
            )
            return resolved_params

        except Exception as e:
            logger.warning(
                f"变量引用解析失败 [步骤 {step_index}]: {str(e)}, 返回原始参数"
            )
            return params

    def _basic_variable_resolution(
        self, params: Dict[str, Any], variable_manager: VariableManager
    ) -> Dict[str, Any]:
        """基础变量解析，简单的字符串替换"""
        import re
    ...
    async def execute_test_case(
        self, test_case: Dict[str, Any], execution_id: str, mode: str = "headless"
    ) -> Dict[str, Any]:
        """
        执行完整的测试用例
        ...
        logger.info(
            f"开始执行测试用例: {test_case.get('name', '未命名')}, 共 {len(steps)} 个步骤"
        )
        ...
        logger.info(
            f"测试用例执行完成: {successful_steps}/{total_steps} 步骤成功, 耗时 {execution_time:.2f}s"
        )
