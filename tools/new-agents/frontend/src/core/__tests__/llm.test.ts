    ...
    describe('后端结构化 Agent Runtime', () => {
        ...
        it('INCIDENT_REVIEW/TIMELINE 应走结构化 Agent Runtime', async () => {
            const artifact = [
                ...
                '| **故障名称** | 支付失败故障 |',
                '| **严重等级** | P1 |',
                '| **发现时间** | 2026-06-22 10:00 |',
                '| **恢复时间** | 2026-06-22 10:30 |',
                '| **持续时长** | 30 分钟 |',
                '| **影响范围** | 支付链路 |',
                '| **当前状态** | 已恢复 |',
                '## 2. 影响量化',
                ...
                JSON.stringify({
                    ...
                    events: [
                        ...
                        },
                        {
                            ...
                            time: '10:30',
                            title: '支付链路恢复',
                            description: '阶段：处理与恢复；关联事实：FACT-001',
                            factIds: ['FACT-001'],
                        },
                    ],
                }, null, 2),
                ...
                '| 类型 | 内容 | 处理方式 | 阻断性 | 状态 |',
                '| --- | --- | --- | --- | --- |',
                '| 事实 | 错误率升高 | 纳入事实摘要 | 否 | 已确认 |',
                '| 推测 | 可能与发布有关 | 根因阶段验证 | 否 | 待确认 |',
                '## 6. 事实摘要',
                '支付链路在 10 点出现错误率升高，10 点 30 分恢复。',
                '## 7. 参与人员',
                '| 角色 | 人员 | 主要行动 | 参与时间 | 状态 |',
                '| --- | --- | --- | --- | --- |',
                '| 一线响应 | SRE | 排查告警 | 10点05分 | 已确认 |',
                '## 8. 待补充信息',
                '| 信息项 | 为什么需要 | 补充方式 | 阻断性 | owner | 状态 |',
                '| --- | --- | --- | --- | --- | --- |',
                '| 影响用户数 | 影响量化 | 查询日志 | 非阻断 | SRE | 待补充 |',
                '## 9. 阶段门禁',
