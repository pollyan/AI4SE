...
const formatContractItems = (title: string, items: string[]): string => (
    items.length === 0
        ? ''
        : [`${title}:`, ...items.map(item => `- ${formatContractItemForPrompt(item)}`)].join('\n')
);

const formatContractItemForPrompt = (item: string): string => (
    item.replace(/\bMermaid\b/gi, '图表')
);
...
const formatArtifactDataContractPrompt = (
    ...
    return [
        '【artifact_data 结构化契约】',
        formatContractItems('模型必须遵守', contract.modelOutputRules),
        formatContractItems('模型不要输出', contract.forbiddenOutputs),
        formatContractItems('后端渲染器负责生成', contract.rendererOutputs),
    ].filter(Boolean).join('\n');
};
...
const removeFormattingInstructions = (description: string): string => (
    description
        .split('\n')
        .filter((line) => !/(Markdown|Mermaid|ai4se-visual|代码块|表格|完整引用|完整保留|产出物必须包含)/i.test(line))
        .join('\n')
        .trim()
);
...
export const buildSystemPrompt = (config: {
    ...
    const cleanArtifact = usesArtifactData
        ...
        : removeMarkTags(currentArtifact);
    ...
    const nextStageGenerationInstruction = nextStage
        ? usesArtifactData
            ? 'stage_action 只表示“请求用户确认进入下一阶段”，不会自动切换阶段；不要在同一轮准备下一阶段业务数据。用户点击前端确认控件后，系统才会切换阶段并自动触发下一阶段生成。'
    ...
    const stageDescription = usesArtifactData
        ? removeFormattingInstructions(currentStage.description)
        : currentStage.description;
    const artifactModeInstruction = usesArtifactData
        ? `【结构化业务数据模式】：当前阶段的右侧成品由后端渲染器生成。你只需要分析业务内容、准备当前阶段需要的结构化业务信息，并在左侧 chat 中说明本轮进展；不要复制、补齐或维护右侧成品正文。`
...
多行内容
    ...
    const artifactUpdatePrinciple = usesArtifactData
        ? '4. **产物数据原则**：本轮只围绕当前阶段业务内容进行分析；不要输出、复制、补齐或维护右侧成品正文，后端会根据结构化业务数据生成右侧产物。'
        : '4. **产出物更新原则**：如果本轮需要更新右侧产出物，必须提供完整、全部的 Markdown 文档内容（包含未修改的部分），绝对不能只输出修改片段，也不能用“...保持不变”省略已有内容。';
    const artifactContext = usesArtifactData
        ? `当前右侧产物参考内容（后端已渲染，仅用于理解上下文，不要复制、补齐或回写成品正文）：
...
\`\`\``;
    ...
    if (stageArtifacts && Object.keys(stageArtifacts).length > 0 && stageIndex > 0) {
        ...
        previousStageArtifacts.forEach(([stageId, artifactContent], index) => {
            ...
            const cleanPreviousArtifact = usesArtifactData
                ? removeFencedCodeBlocks(removeMarkTags(artifactContent))
                : removeMarkTags(artifactContent);
