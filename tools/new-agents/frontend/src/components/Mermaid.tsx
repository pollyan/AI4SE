...
export const Mermaid: React.FC<MermaidProps> = ({
  ...
  }, [blockIndex, onRenderError, onRenderSuccess]);
    ...
    const renderChart = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          latestCallbacksRef.current.onRenderSuccess?.(
            latestCallbacksRef.current.blockIndex ?? 0
          );
        }
        ...
        latestCallbacksRef.current.onRenderError?.({
          code: chart,
          message: errorMessage,
          blockIndex: latestCallbacksRef.current.blockIndex ?? 0,
        });
      }
    };
