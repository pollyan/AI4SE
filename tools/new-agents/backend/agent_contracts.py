...
WORKFLOW_STAGES: dict[str, list[str]] = get_workflow_stages()
...
def build_artifact_contract_prompt(
    *,
    ...
    return (
        ...
        "固定栏目、固定字段模板或固定 bullet 数量。如果当前阶段可推进，应自然引导用户查看右侧产出物，"
        "并由 stage_action 触发前端确认控件。\n"
        "本阶段必须更新右侧产出物：artifact_update.type 必须为 replace。\n"
        "artifact_update.markdown 必须是完整 Markdown 文档，不能只返回片段，"
        "不能用省略号表示未修改内容。\n"
        "上述 artifact_update.markdown 契约仅适用于 artifact_update.markdown；"
        "如果运行时追加 artifact_data 结构化输出要求，以 artifact_data 要求为准。\n"
