...
def test_real_pydantic_ai_runtime_returns_valid_clarify_artifact():
    ...
    assert "## 1. 被测系统与边界" in output.artifact_update.markdown
    assert "## 2. 系统交互与核心链路" in output.artifact_update.markdown
    assert "## 3. 待澄清与阻断性问题" in output.artifact_update.markdown
    assert "## 4. 隐式需求与非功能性考量" in output.artifact_update.markdown
