...
class TestAIOnlyAutomation:
    ...
    def test_ai_data_extraction(self):
        ...
        print(
            f"✅ AI提取了 {len(search_results) if isinstance(search_results, list) else '1个'} 条搜索结果"
        )
    ...
    def test_ai_multi_step_workflow(self):
        ...
        print(
            f"✅ 第一次搜索结果: {len(first_results) if isinstance(first_results, list) else '已获取'}"
        )
        ...
        print(
            f"✅ 第二次搜索结果: {len(second_results) if isinstance(second_results, list) else '已获取'}"
        )
