...
class TestImprovedWorkflow:
    ...
    def test_robust_multi_step_workflow(self):
        ...
        # 使用新的智能等待方法
        if self.ai.smart_wait_and_verify("页面显示了机器学习相关的搜索结果", max_wait=8):
            print("✅ 第一次搜索成功")
        else:
        ...
        try:
            ...
            print(f"✅ 第一次搜索提取了 {len(first_results) if isinstance(first_results, list) else 1} 条结果")
        ...
        # 等待第二次搜索完成
        if self.ai.smart_wait_and_verify("页面显示了深度学习相关的搜索结果", max_wait=8):
            print("✅ 第二次搜索成功")
        else:
        ...
        try:
            ...
            print(f"✅ 第二次搜索提取了 {len(second_results) if isinstance(second_results, list) else 1} 条结果")
        except Exception as e:
