...
class TestCase(db.Model):
    ...
    executions = db.relationship(
        "TestExecution", backref="test_case", lazy=True, cascade="all, delete-orphan"
    )
...
class AITestService:
    ...
    def execute_test_case(self, test_case, execution_id):
        ...
        if not ai:
            return {"success": False, "error": "AI服务初始化失败"}

        try:
            # 解析测试步骤
            steps = (
                json.loads(test_case.generated_steps)
                if test_case.generated_steps
                else []
            )
            ...
            for i, step in enumerate(steps):
                ...
                try:
                    ...
                    if step["type"] == "goto":
                        ...
                        result = ai.ai_input(
                            step["params"]["text"], step["params"]["locate_prompt"]
                        )
                        step_result["result"] = result
                        step_result["success"] = True
            ...
            return {"success": True, "results": results, "screenshots": screenshots}
...
def create_test_case():
    ...
    steps = ai_service.parse_natural_language_to_steps(
        data["natural_language_input"], data.get("target_url")
    )
...
def execute_test_case(case_id):
    ...
    execution = TestExecution(
        test_case_id=case.id, execution_id=execution_id, status="running"
    )
    ...
    try:
        ...
        return jsonify(
            {"execution_id": execution_id, "status": execution.status, "result": result}
        )
        ...
        return (
            jsonify(
                {"execution_id": execution_id, "status": "failed", "error": str(e)}
            ),
            500,
        )
