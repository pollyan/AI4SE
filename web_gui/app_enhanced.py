...
try:
    ...
    from web_gui.database_config import (
        get_flask_config,
        print_database_info,
        validate_database_connection,
    )
...
def register_error_handlers(app):
    ...
    def handle_validation_error(e):
        """处理验证异常"""
        return jsonify({"code": 400, "message": e.message, "details": e.details}), 400

    @app.errorhandler(NotFoundError)
    def handle_not_found_error(e):
        """处理资源不存在异常"""
        return jsonify({"code": 404, "message": e.message, "details": e.details}), 404

    @app.errorhandler(DatabaseError)
    def handle_database_error(e):
        """处理数据库异常"""
        return jsonify({"code": 500, "message": e.message, "details": e.details}), 500
