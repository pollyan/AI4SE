...
def debug_screenshot_history():
    ...
    file_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "debug_screenshot_history.html"
    )
...
def handle_stop_execution(data):
    ...
    if execution_id:
        # TODO: 实现停止执行逻辑
        emit(
            "execution_stopped", {"execution_id": execution_id, "message": "执行已停止"}
        )
    else:
    ...
    try:
        ...
        with app.app_context():
            ...
            if not execution:
                socketio.emit(
                    "execution_error",
                    {"execution_id": execution_id, "message": "执行记录不存在"},
                    room=client_sid,
                )
                return
            ...
            try:
                ...
                class FallbackMockAI:
                    ...
                    def ai_scroll(
                        self, direction="down", scroll_type="once", locate_prompt=None
                    ):
                        print(f"[模拟] 滚动: {direction} ({scroll_type})")
                        time.sleep(0.5)
                    ...
                    def take_screenshot(self, title):
                        ...
                        try:
                            ...
                            draw = ImageDraw.Draw(img)
                            draw.rectangle(
                                [50, 50, 750, 550], outline="darkblue", width=3
                            )
                            draw.text(
                                (100, 100), "Fallback Mock Screenshot", fill="darkblue"
                            )
                            draw.text(
                                (100, 150),
                                f"URL: {getattr(self, 'current_url', 'Unknown')}",
                            ...
                            os.makedirs(screenshot_dir, exist_ok=True)
                            with open(
                                screenshot_path.replace(".png", ".txt"), "w"
                            ) as f:
                                f.write(
                                    f"Fallback Mock Screenshot - {time.strftime('%Y-%m-%d %H:%M:%S')}"
                                )
                        except Exception as e:
                            print(f"[模拟] 截图保存失败: {e}")
                            os.makedirs(screenshot_dir, exist_ok=True)
                    ...
                    socketio.emit(
                        "step_started",
                        {
                            ...
                            "step_description": step.get(
                                "description", step.get("action", f"步骤 {i+1}")
                            ),
        ...
        socketio.emit(
            "execution_error",
            {"execution_id": execution_id, "message": f"执行过程中发生错误: {str(e)}"},
            room=client_sid,
        )
