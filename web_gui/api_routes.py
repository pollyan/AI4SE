...
def create_testcase():
    ...
    try:
        ...
        if not data.get("name"):
            return jsonify({"code": 400, "message": "测试用例名称不能为空"}), 400

        if not data.get("steps"):
            return jsonify({"code": 400, "message": "测试步骤不能为空"}), 400
        ...
        return jsonify(
            {"code": 200, "data": testcase.to_dict(), "message": "测试用例创建成功"}
        )
    except Exception as e:
        db.session.rollback()
...
def get_testcase(testcase_id):
    ...
    try:
        ...
        return jsonify({"code": 200, "data": testcase.to_dict()})
    except Exception as e:
        return jsonify({"code": 500, "message": f"获取失败: {str(e)}"}), 500
...
def update_testcase(testcase_id):
    ...
    try:
        ...
        return jsonify(
            {"code": 200, "data": testcase.to_dict(), "message": "测试用例更新成功"}
        )
    except Exception as e:
        db.session.rollback()
...
def delete_testcase(testcase_id):
    ...
    try:
        ...
        return jsonify({"code": 200, "message": "测试用例删除成功"})
    except Exception as e:
        db.session.rollback()
...
def create_execution():
    ...
    try:
        ...
        return jsonify(
            {
                "code": 200,
                "data": {"execution_id": execution_id, "status": "pending"},
                "message": "执行任务创建成功",
            }
        )
    except Exception as e:
        db.session.rollback()
        return jsonify({"code": 500, "message": f"创建执行任务失败: {str(e)}"}), 500
...
def get_execution_status(execution_id):
    ...
    try:
        ...
        return jsonify({"code": 200, "data": execution_data})
    except Exception as e:
        return jsonify({"code": 500, "message": f"获取执行状态失败: {str(e)}"}), 500
...
def get_executions():
    ...
    try:
        ...
        return jsonify({"code": 500, "message": f"获取执行历史失败: {str(e)}"}), 500
...
def get_templates():
    ...
    try:
        ...
        return jsonify({"code": 200, "data": [t.to_dict() for t in templates]})
    except Exception as e:
...
def create_template():
    ...
    try:
        ...
        return jsonify(
            {"code": 200, "data": template.to_dict(), "message": "模板创建成功"}
        )
    except Exception as e:
        db.session.rollback()
        return jsonify({"code": 500, "message": f"创建模板失败: {str(e)}"}), 500
...
def get_dashboard_stats():
    ...
    try:
        ...
        success_executions = ExecutionHistory.query.filter(
        ...
        # 成功率
        success_rate = (
            (success_executions / total_executions * 100) if total_executions > 0 else 0
        )
