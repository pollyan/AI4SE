...
class ExecutionService:
    ...
    def execute_testcase_async(self, testcase_id: int, mode: str = 'headless') -> str:
        ...
        thread = threading.Thread(
            target=self._execute_testcase_thread,
    ...
    def _execute_testcase_thread(self, execution_id: str, testcase: TestCase, mode: str):
        """执行测试用例的线程函数"""
        ai = None
        try:
            # 获取AI服务
    ...
    def _execute_single_step(self, ai, step: Dict, mode: str, execution_id: str, step_index: int) -> Dict:
        """执行单个测试步骤"""
        try:
    ...
    def _handle_skipped_step(self, execution_id: str, step_index: int, step: Dict):
        ...
        socketio.emit('step_skipped', {
            'execution_id': execution_id,
            'step_index': step_index,
            'step_description': step.get('description', step.get('action', f'步骤 {step_index + 1}')),
            'message': '此步骤已被标记为跳过'
        })
    ...
    def _handle_step_error(self, execution_id: str, step_index: int, step: Dict, error_message: str):
        """处理步骤错误"""
        step_execution = StepExecution(
            execution_id=execution_id,
            step_index=step_index,
