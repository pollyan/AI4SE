...
class AITestCaseParser:
    ...
    def __init__(self):
        ...
        self.step_patterns = {
            ...
            "wait": [r"等待|等候", r"加载|完成|出现", r"直到.*?显示"],
            "assert": [
                r"验证|检查|确认|断言",
                r"应该|必须|需要",
                r"页面.*?显示|包含.*?内容",
            ...
            "scroll": [r"滚动|翻页|下拉|上拉", r"向.*?滚动", r"滚动到.*?位置"],
        }
    ...
    def parse_with_ai(
        self, natural_input: str, target_url: str = None
    ) -> List[Dict[str, Any]]:
        """
        使用AI模型解析自然语言描述
...
"""
    ...
    def _parse_scroll_step(self, line: str, line_num: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "params": {"direction": direction, "scroll_type": scroll_type},
        }

    def enhance_steps_with_context(
        self, steps: List[Dict[str, Any]], context: Dict[str, Any] = None
    ) -> List[Dict[str, Any]]:
        """
        根据上下文增强测试步骤
        ...
        for i, step in enumerate(steps):
            ...
            if step["type"] in ["ai_tap", "ai_input"] and i < len(steps) - 1:
                ...
                if (
                    next_step["type"] in ["ai_assert", "ai_query"]
                    and next_step["type"] != "ai_wait_for"
                ):
        ...
        for i, step in enumerate(steps):
            ...
            if step_type == "goto":
                ...
                if "text" not in params or "locate_prompt" not in params:
                    errors.append(
                        f"步骤 {step_num}: ai_input 操作缺少 'text' 或 'locate_prompt' 参数"
                    )
                ...
                if "prompt" not in params:
                    errors.append(
                        f"步骤 {step_num}: {step_type} 操作缺少 'prompt' 参数"
                    )
...
def parse_natural_language(
    natural_input: str, target_url: str = None, use_ai: bool = True
) -> List[Dict[str, Any]]:
    """
    解析自然语言测试描述的便捷函数
